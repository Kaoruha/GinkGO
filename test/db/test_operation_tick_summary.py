...
class OperationTickSummaryTest(unittest.TestCase):
    ...
    def test_Operationtick_summary_delete(self) -> None:
        for i in self.params:
            ...
            delete_tick_summary(res["uuid"])
            time.sleep(0.1)
            size2 = get_table_size(self.model)
            # Check that table size decreased (record was deleted)
            self.assertLess(size2, size1, f"Table size should decrease after delete. Before: {size1}, After: {size2}")
    ...
    def test_Operationtick_summary_softdelete(self) -> None:
        for i in self.params:
            ...
            softdelete_tick_summary(res["uuid"])
            time.sleep(0.05)
            size2 = get_table_size(self.model)
            # Check that table size decreased (record was deleted - softdelete uses delete for ClickHouse)
            self.assertLess(size2, size1, f"Table size should decrease after softdelete. Before: {size1}, After: {size2}")

            # Note: softdelete_tick_summary actually calls delete_tick_summary for ClickHouse,
            # so the record is already deleted and we don't need to call delete again
    ...
    def test_Operationtick_summary_get(self) -> None:
        ...
        size1 = get_table_size(self.model)
        self.assertEqual(self.count, size1 - size0)
        df = get_tick_summaries_page_filtered(code=code)
