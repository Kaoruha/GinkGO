...
class OperationAdjustfactorTest(unittest.TestCase):
    ...
    def test_OperationAdjustfactor_delete(self) -> None:
        for i in self.params:
            size0 = get_table_size(self.model)
            ...
            delete_adjustfactor(item.uuid)
            size2 = get_table_size(self.model)
            self.assertEqual(-1, size2 - size1)

    def test_OperationAdjustfactor_softdelete(self) -> None:
        for i in self.params:
            size0 = get_table_size(self.model)
            item = add_adjustfactor(**i)
            size1 = get_table_size(self.model)
            ...
            softdelete_adjustfactor(item.uuid)
            size2 = get_table_size(self.model)
    ...
    def test_OperationAdjustfactor_update(self) -> None:
        for i in self.params:
            size0 = get_table_size(self.model)
