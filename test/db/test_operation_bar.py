...
class OperationBarTest(unittest.TestCase):
    ...
    def setUpClass(cls):
        ...
        cls.params = [
            {
                "code": uuid.uuid4().hex,
                "open": Decimal(str(round(random.uniform(1.5, 5.5), 2))),
                "high": Decimal(str(round(random.uniform(1.5, 5.5), 2))),
                "low": Decimal(str(round(random.uniform(1.5, 5.5), 2))),
                "close": Decimal(str(round(random.uniform(1.5, 5.5), 2))),
                "volume": random.randint(100, 1000),
                "amount": Decimal(str(round(random.uniform(1.5, 5.5), 2))),
    ...
    def test_OperationBar_delete(self) -> None:
        for i in self.params:
            size0 = get_table_size(self.model)
            df = add_bar(**i)
            uuid = df.uuid
    ...
    def test_OperationBar_softdelete(self) -> None:
        for i in self.params:
            size0 = get_table_size(self.model)
            df = add_bar(**i)
            uuid = df.uuid
