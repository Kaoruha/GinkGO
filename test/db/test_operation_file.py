...
class OperationFileTest(unittest.TestCase):
    ...
    def test_OperationFile_get_fuzz(self) -> None:
        ...
        size1 = get_table_size(self.model)
        time.sleep(0.01)
        df = get_files_page_filtered(type=FILE_TYPES.STRATEGY)
        self.assertEqual(df.shape[0] >= 2, True)
        print(df)
        df2 = get_files_page_filtered(name=f"{name_prefix}")
        print(df2)

    def test_OperationFile_get_content(self) -> None:
        content = uuid.uuid4().hex
        res = add_file(type=FILE_TYPES.STRATEGY, name="contentgettest", data=content.encode())
        b_content = get_file_content(res.uuid)
        self.assertEqual(b_content, content.encode())
