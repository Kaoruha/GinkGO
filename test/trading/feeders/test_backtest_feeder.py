...
class TestBacktestFeederConstruction:
    ...
    def test_default_constructor(self):
        ...
        # 继承自TimeRelated的属性
        assert feeder.now is None, "初始时间应该为None（未传timestamp参数）"
    ...
    def test_dual_inheritance(self):
        ...
        # 验证BaseFeeder的接口（通过BacktestBase和TimeRelated）
        assert hasattr(feeder, 'set_backtest_ids'), "BacktestBase方法"
        assert hasattr(feeder, 'now'), "TimeRelated属性"
        assert hasattr(feeder, 'timestamp'), "TimeRelated属性"
        assert hasattr(feeder, 'bar_service'), "BaseFeeder的DI属性"

        # 验证IBacktestDataFeeder的接口
        assert hasattr(feeder, 'advance_to_time'), "IBacktestDataFeeder核心方法"
        assert hasattr(feeder, 'get_historical_data'), "IBacktestDataFeeder数据方法"
        assert hasattr(feeder, 'validate_time_access'), "IBacktestDataFeeder时间验证"
...
class TestTimeControl:
    ...
    def test_set_time_controller_method(self):
        """测试set_time_controller绑定"""
        feeder = BacktestFeeder()

        # 创建LogicalTimeProvider
        time_provider = LogicalTimeProvider(datetime(2023, 1, 1))

        # 调用set_time_controller(time_controller)
        feeder.set_time_controller(time_provider)
    ...
    def test_time_boundary_validator_creation(self):
        ...
        # 调用set_time_controller()自动创建validator
        time_provider = LogicalTimeProvider(datetime(2023, 1, 1))
        feeder.set_time_controller(time_provider)
    ...
    def test_advance_time_callback(self):
        ...
        # 验证self.now被更新（now是当前回测时间）
        assert feeder.now == target_time

        # 注：timestamp是实体数据的创建时间，不由advance_time更新
