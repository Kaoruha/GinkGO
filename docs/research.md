...
**1yr calendar-2025 fallback（进行中）** BT `945c5716c5154fac9625bf728f4e072b`：同配置（MomentumSelector interval=5/rank=5/window=20 + momentum lookback=10/thr=0.022 + RatioSizer 0.1 + LossLimitRisk 10%），窗口 **2025-01-01~12-31**（全年，避开 2026 最密尾段提速）。`timeout 3000`（50min 余量）。04:01:22 起跑，结果待下轮记录。
...
> **待 ITER-003 1yr cal-2025（BT `945c5716`）完成即按结果分流**：
> - **若盈利（年化>0）**：立即起 **ITER-002 walk-forward**——2024-07~2025-06 训练 + 2025-06~2026-07 OOS 验证（切两段，验 G2 稳定 + G4 无过拟合）。用同 portfolio 建两 bt，bg 跑。
> - **若亏损（承袭 smoke 10% 胜率）**：起 **ITER-003b plan-B**（动态 selector 仍守 G4，换配置）：
>   - B1 `momentum_selector(window=60,rank=5)` + `momentum(lookback=20,threshold=0.05)`——拉长窗口滤短期反转、提高阈只买强动量。
>   - B2 `popularity_selector` + `momentum`——换热点股池（避买顶部赢家）。
>   - ⚠️ `trend_follow` 有 `arch_trend_follow_calendar_day_window_zero_signal`（日历日/交易日窗口 bug 致 0 信号），**不用**它作 plan-B 除非先验修复。
