...
> - **下一轮方向（优先级）**：
>   1. **ITER-004 momentum walk-forward**（唯一研究线）：`momentum_selector+momentum` 跑 **2024 全市场 1yr**（含 924 大涨，momentum 最可能盈利的窗口；若 2024 仍亏→momentum 基本可判死）。⚠️ **并发 gate**：1yr 全市场 ~50min，须先 pgrep 确认无并发回测（本轮 bt `a3778c58` 他 job iter10 并发中，起前等清空）；起后 bg+watcher 跨轮 poll。
>   2. ~~探测 cn_all/multi_params~~（ITER-003d 已完成：均不可用）。
>   3. ITER-004 若亏 → 结论阶段：现有动态 selector×内置策略组合在近 3 年 A 股难稳定盈利，记录结论收束本 arc。
