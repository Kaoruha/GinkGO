...
> - **现状盘点**：能跑通产信号的**动态 selector 目前仅 `momentum` 一个**（popularity 死于 #6760，cn_all/multi_params 未测）。真实可用策略（非 stub）约：momentum/mean_reversion/moving_average_crossover/dual_thrust/trend_reverse/random_signal 等（scalping/price_action 是 stub，trend_follow 0 信号 bug）。
...
> - **下一轮方向（优先级）**：
>   1. **ITER-004 momentum walk-forward**：`momentum_selector+momentum` 跑 **2023 & 2024 各 1yr**（或 2023-2024 合并 2yr），看是否存盈利窗口。这是唯一已验证产信号的动态组合，最值得定论。
>   2. 探测 `cn_all`/`multi_params` selector 是否产信号（补 selector 选项，给 MR/trend_reverse 等喂票）。
>   3. 上述若全亏 → 结论阶段：现有动态 selector×内置策略组合在近 3 年 A 股难稳定盈利，记录结论收束本 arc。
