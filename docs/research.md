...
- **⚠️ perf 观察**：全市场 momentum_selector 2yr 极慢（47%@21min，后段 universe 增大每月更慢），ETA ~04:19 逼近 60min timeout，**大概率超时卡死**。教训：**全市场长窗口回测单次 >60min，迭代不可行**。后续迭代用 1yr 窗口（fallback 脚本 `iter003_1yr_fallback.sh` 已备），优胜者再做完整 2yr 终验。
...
> **待 ITER-003 全 2 年周期（BT `fa9a1ca9`）完成即按结果分流（监视器 `bviinqkov` 守完成）**：
