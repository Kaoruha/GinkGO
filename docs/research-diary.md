...
## 贵州茅台(600519.SH) 2018年测试 2026-05-26

### 股票特征
- 2018年收盘: ~540元（从年初~700元下跌）
- 年度表现: -23%（熊市）

### 测试设置
- 策略: MovingAverageCrossoverStrategy (MA5, MA20)
- 选股: FixedSelector (600519.SH)
- 仓位: FixedSizer (100股)
- 风控: NoRisk
...
### 分析
1. **MA策略2018年亏损-1.49%**：跑赢大盘（茅台2018年下跌~23%）
2. **超额收益显著**：相对买入持有超额收益约+21.5%
3. **仅3笔订单**：信号17次但只成交3笔，可能因为T+1或开盘价限制
4. **最大回撤3.17%**：风控有效，避免了大跌

### 关键发现
1. **MA策略是熊市保护盾**：2018年茅台下跌23%，但MA策略仅亏损1.49%
2. **验证"动量策略熊市武器"规律**：结合之前601012.SH测试结果
   - 2018熊市: Momentum策略表现优异（-5.08% vs 大盘-53%）
   - MA交叉策略也有效（-1.49% vs 大盘-23%）
3. **股票代码大小写问题**：数据库存储为"600519.SH"(大写)，CLI参数需用大写

### 发现的问题

#### CLI回测持久化问题
- **现象**: backtest状态显示"running"但实际已完成
- **影响**: 无法通过`ginkgo backtest cat`查看已完成回测的Results
- **解决**: 使用`ginkgo backtest list`查看所有回测，状态"completed"的可以查看结果

#### 代码大小写敏感
- **现象**: `600519.sh`(小写)找不到数据，`600519.SH`(大写)有243条
- **影响**: CLI参数使用小写时回测0信号
- **解决**: 使用大写股票代码
...
| 市场 | 策略 | 表现 | 原因 |
|------|------|------|------|
| 2018熊市 | Momentum(20,2%) | **超额+47%** | 下跌趋势明确，动量信号有效 |
| 2019牛市 | Momentum(20,2%) | **超额-12%** | 持续上涨导致动量衰减 |
| 2020COVID | TrendMomentum | **超额+0.56%** | 波动市场中趋势跟踪有效 |
| 2020COVID | MA(5,20) | **超额-0.39%** | 震荡市中假信号多 |

### 2. 策略适用场景

| 策略 | 最佳场景 | 最差场景 |
|------|----------|----------|
| TrendMomentum | 趋势市场/强势股 | 震荡市/弱势股 |
| RSI Bounce | 震荡市/均值回归 | 趋势市场/强势股 |
| MA Cross | 明确趋势 | 震荡市 |

### 3. 股票选择优先级

**TrendMomentum + 强势股**：
- 601012.SH (隆基绿能): 年化+72.66%
- 601899.SH (紫金矿业): 年化+21.36%
- 000858.SZ (五粮液): 年化+120.96%

### 下一步研究
1. [ ] 测试600519.SH在不同策略下的表现
2. [ ] 验证"大小写问题"的根本原因和修复方案
3. [ ] 扩展测试年份：2019/2020/2021
4. [ ] 研究市场环境自动识别方法

## 贵州茅台(600519.SH) 因子IC分析 2026-05-26

### 年度因子IC分析

| 年份 | 买入持有 | IC(5日动量) | IC(20日动量) | IC(波动率) | IC(sma_ratio) |
|------|----------|-------------|-------------|------------|----------------|
| 2018 | **-16.17%** | +0.406 | +0.181 | -0.039 | +0.367 |
| 2019 | **+97.50%** | +0.412 | +0.191 | +0.021 | +0.388 |
| 2020 | **+76.81%** | +0.434 | +0.211 | -0.062 | +0.385 |
| 2021 | **+2.65%** | +0.478 | +0.206 | -0.090 | +0.373 |
| 2022 | **-15.81%** | +0.413 | +0.210 | +0.079 | +0.375 |
...
4. **volatility_20 IC接近零**
   - 大多数年份IC接近0，说明波动率对茅台预测能力弱
   - 高波动不等于低收益
...
|------|--------|--------|
| **sma_ratio** | **+0.378** | ⭐⭐⭐ 最稳定 |
| momentum_5 | +0.429 | ⭐⭐⭐ 最稳定 |
| momentum_20 | +0.200 | ⭐⭐ 中等 |
| volatility_20 | -0.018 | ❌ 不稳定 |

### 策略建议

基于茅台因子分析:
1. **推荐sma_ratio因子**: 最稳定，可用于选股
2. **短期动量 momentum_5**: IC最高的因子
3. **避免单独使用波动率因子**: 对茅台预测能力弱

### CLI回测限制发现

**问题**: backtest状态显示"running"但实际已完成，Results区域为空
- 影响: 无法通过`ginkgo backtest cat`查看已完成回测的完整Results
- 解决: 需要等待更长时间或使用其他方式验证

## 多股票因子IC对比 (2020年)

### 强势股因子IC排名

| 股票 | 代码 | 2020涨幅 | IC(5日动量) | IC(20日动量) | IC(sma_ratio) |
|------|------|----------|-------------|-------------|----------------|
| 隆基绿能 | 601012.SH | **+247.5%** | +0.474 | +0.271 | +0.403 |
| 五粮液 | 000858.SZ | +121.0% | +0.439 | +0.192 | +0.350 |
| 紫金矿业 | 601899.SH | +95.6% | +0.423 | +0.203 | +0.383 |
| 贵州茅台 | 600519.SH | +76.8% | +0.441 | +0.205 | +0.378 |

### 关键发现

1. **强势股IC更高**: 隆基绿能(+247.5%)的IC全面高于其他股票
2. **momentum_5最强**: 所有股票5日动量IC都在+0.42以上
3. **sma_ratio稳定**: 所有股票IC都在+0.35-+0.40之间
4. **IC与涨幅正相关**: 股票涨幅越高，各因子IC也越高

### 因子有效性排序

| 因子 | 平均IC | 结论 |
|------|--------|------|
| momentum_5 | +0.444 | ⭐ **最强因子** |
| sma_ratio | +0.379 | ⭐ 稳定有效 |
| momentum_20 | +0.218 | 中等有效 |

### 结论

1. **强势股具有更强的动量效应**: 最好的股票不仅涨幅大，动量也更强
2. **短期动量(5日)优于长期动量(20日)**: 说明A股强势股动量持续时间短
3. **sma_ratio是选股的有效因子**: 可以用来筛选强势股
...
| 配置 | Final Value | 年化收益 | 夏普 | 最大回撤 | 订单数 |
|------|-------------|---------|------|---------|-------|
| Momentum (无止损) | ¥92,772 | -5.08% | -1.86 | 8.26% | 19 |
| Momentum + 10%止损 | ¥92,742 | -5.10% | -1.87 | 8.29% | 19 |
...
### 完整策略对比总结 (2018年 601012.SH)

| 策略 | Final | 年化 | 夏普 | 回撤 | 信号 | 订单 |
|------|-------|------|------|------|------|------|
| B&H | ¥47,300 | -53% | - | - | 0 | 0 |
| Momentum | ¥92,772 | -5.08% | -1.86 | 8.26% | 30 | 19 |
| Momentum+止损 | ¥92,742 | -5.10% | -1.87 | 8.29% | 30 | 19 |
| MA Cross | ¥95,502 | -3.14% | -2.91 | 4.54% | 17 | 14 |

### 核心结论

1. **熊市最佳策略: MA Cross (-3.14%)**
   - 比Momentum少亏损2%
   - 比B&H少亏损50%

2. **止损在熊市无效**
   - 原因: 卖出后资金闲置，错过后续更低点的买入机会
...
4. **多股票验证**:
   - 000001.SZ 2018: -1.81% (vs -35% B&H, +33%超额)
   - 规律与601012.SH一致

**核心结论**:
- 动量策略是"熊市武器，牛市陷阱"
- MA Cross优于Momentum在熊市表现
- 止损在单边市中无效
