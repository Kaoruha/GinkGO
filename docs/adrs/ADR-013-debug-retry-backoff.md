...
- **为何保留语义不改装饰器**：`@retry` 被 ~15+ 处使用（`tick_service` ×5、`ginkgo_kafka` ×3、`stockinfo`/`param`/`adjustfactor`/`tushare` …），改 debug 分支牵动全系统；且 debug 下跳过退避是刻意的测试加速设计。
...
- **难逆转**：debug 改变 retry 语义是隐式跨系统耦合，~15+ 调用点依赖现状。
- **反直觉**：一个"调试开关"（debug）竟影响生产 API 封禁风险——本 ADR 即该反直觉行为的锚点，防未来把"debug 跳过退避"当 bug 误改。
- **验证方式**：批量同步封禁风险验证见 `docs/verify-tushare-sync-and-schedule-2026-06-19.md`；单股同步安全（retry 不触发）。
