...
**Status:** Accepted
**Date:** 2026-07-26
**关联:** 修订 ADR-010 §4(钩子处理条款)· 承 ADR-009 line 28(Base 保护)的 ADR 背书例外 · ADR-025 原则 1 / 步骤 ⑤(DB Mapper 收尾)· Epic E #6701(CRUD/Mapper seam 集中化)· 审计 issue #6629
...
**现状(2026-07-26,#6629 审计核实,3 路并行交叉验证)**:
...
| 子类 override | 30+ CRUD 重写这些钩子做各自 Entity↔ORM 转换 |
| **外部调用方** | **零**(ModelList / CRUDResult 两路已断) |

ADR-025(2026-07-24)已立 **Mapper 家族覆盖四边界**,其中 DB 边界 `XxxMapper` 承担 ORM↔Entity↔DTO。但 11 个 Mapper(master 实存)**只有 `from_model`(ORM→Entity 出路),缺 `to_model`(Entity→ORM 入路)**——故 28 CRUD 入库仍走 `_convert_input_item` 钩子,钩子族无法退役。

**死活混杂治理僵局**:钩子族里有的活(内部调用 + 子类 override)、有的死(外部调用方已断);ADR-010 §4「留作 dead code」+ ADR-009 line 28「禁擅改 Base」锁死不可删 → 治理卡住。case-by-case 标 `deprecated` 既不统一又难追踪(用户反馈:「为了架构,应该统一,别区分那么细了」)。
...
退役范围:`_conversion.py` 钩子族 + 30+ 子类 override + `tick_crud` 等私有 `_convert_to_*`(审计确认全为 Entity↔ORM 转换职责,无其他语义)。
...
> CRUD mixin 转换钩子族(`_convert_to_business_objects` 等)**授权退役**,Entity↔ORM 转换统一由 Mapper 家族承担。退役前提:Mapper 补 `to_model` 反向入路。详见 ADR-029。
...
- **为何现在收敛(非停在 ADR-010 §4「留 dead code」)**:ADR-010 当时留 dead code 是因条件未备——ModelList 全删触 Base、Mapper 家族未立。此后 ADR-025 Mapper 家族覆盖四边界 + Service 多出口已落实 + Task 4.1 / `8b32a25f` 断开外部调用方,**收敛条件已成熟**。继续「留 dead code」= 持续维护死活混杂的认知税。
- **为何整体退役非一钩子一判**:钩子族职责单一(Entity↔ORM 转换),无混合语义;case-by-case 标 `deprecated` 既不统一(用户已拒)又难追踪。一个机制管全部,呼应 ADR-022 原则 3。
- **为何需 Mapper 补 `to_model` 前提**:当前 Mapper 仅 `from_model` 出路,缺 `to_model` 入路;入库是 28 CRUD 的活跃路径,退役入库钩子(`_convert_input_item`)前必须先有 Mapper 替代,否则断入库。
...
- **转换机制单一化**:Entity↔ORM 全走 Mapper,DB 边界转换真相源唯一,消除 30+ 子类重复 override。
- **与 ADR-025 步骤 ⑤ 合流**:本 ADR 是步骤 ⑤(DB Mapper 收尾)的授权前置;步骤 ⑤ 执行 = 本 ADR §4 前提 1 + §5 顺序落地。
