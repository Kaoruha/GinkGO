...
| ADR-029 | [Entity↔ORM 全链路收敛到 Mapper,钩子族退役(不留 dead code)](ADR-029-basecrud-hook-retirement.md) | Accepted | 2026-08-02 |
...
| ADR-046 | [回测任务生命周期状态机形式化（迁移表+所有权+崩溃窗口+幂等）](ADR-046-backtest-task-lifecycle-state-machine.md) | Accepted（分阶段实现） | 2026-08-15 |
