...
### 2. 源码组件与数据库组件的关系（已知设计）
回测引擎从数据库 File 表加载组件源码，不读取源码文件。这是 **设计决策**：
- 系统安装时自动将 `src/` 下的组件注入数据库
- 修改源码后需通过 `ginkgo component edit` 更新数据库版本，或重新安装
- 新增组件时需注意同步数据库版本（如 RatioSizer 源码已实现但数据库仍为空）
- **建议**：添加 `ginkgo component sync` 命令简化源码→数据库同步流程
...
| 功能 | 说明 | 优先级 |
|------|------|--------|
| ~~`backtest compare <id1> <id2>`~~ | ~~并排对比结果~~ | ~~P1~~ ✅ `ginkgo compare` |
...
| 命令 | 功能 |
|------|------|
| `ginkgo data get stockinfo` | 查看股票信息（交互式分页） |
| `ginkgo data get day --code 000001.SZ` | 查看日线 OHLCV 数据 |
| `ginkgo data get tick --code 000001.SZ` | 查看 Tick 数据 |
| `ginkgo data sync day --code 000001.SZ` | 同步日线数据（智能/全量） |
| `ginkgo data sync tick` | 同步 Tick 数据 |
| `ginkgo data sync stockinfo` | 同步股票信息 |

### 未实现的命令
```
ginkgo data get adjustfactor  → "not yet implemented"
ginkgo data get calendar      → "Unknown data type"（help 中列出但代码不识别）
ginkgo data get sources       → "not yet implemented"
ginkgo data status            → "not yet implemented"
```

### 已清理
- `core_cli.py` 中不可用的 `show`/`plot` 函数已删除（依赖不存在的 `plot_daybar`/`plot_tick`）
...
2. **部分数据查看未实现**：复权因子、交易日历、数据源状态等辅助数据仍无法通过 CLI 查看
3. **组件源码需手动同步数据库**：修改源码后需手动更新数据库版本，新增组件易遗漏（如 RatioSizer）
4. **策略搜索效率低**：5 步重复操作、无批量清理，52 轮迭代成本极高（`ginkgo compare` 已可用）

### 一句话总结
Ginkgo 的架构和组件设计已经成熟，但回测引擎的可靠性问题是最大障碍。CLI 经整理后功能覆盖面显著提升：数据查看（`data get day/tick/stockinfo`）、回测对比（`compare`）、交易记录（`record`）、开发工具（`dev`/`test`/`cache`）均已可用。剩余短板：复权因子/交易日历查看未实现，回测引擎非确定性 Bug 待修复。
...
- **CLI 整理**：删除 9 个低价值/stub CLI 模块，注册 5 个已实现模块
- **data get day/tick**：实现 K 线和 Tick 数据查看
- **core_cli.py**：删除不可用的 show/plot 死代码
- **新注册命令**：`ginkgo compare`（回测对比）、`ginkgo record`（交易记录）、`ginkgo dev`（开发工具）、`ginkgo test`（测试运行器）、`ginkgo cache`（缓存管理）
