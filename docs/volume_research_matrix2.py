...
def event_handler(event: QEvent, df: DataFrame) -> DataFrame:
    df_new = df
    if event.event_type == "s":
        item = {"rate": event.rate, "signal_day": event.signal, "signal_count": 1}
        df_new = df_new.append(item, ignore_index=True).fillna(0)
    elif event.event_type == "o":
        item = {
            "rate": event.rate,
            "signal_day": event.signal,
        }
        item["observe_" + str(event.observe)] = 1 if event.profit > 0 else 0
        item["profit_" + str(event.observe)] = event.profit
        df_new = df_new.append(item, ignore_index=True).fillna(0)
...
def q_handle(config):
    ...
    while stocks_done.qsize() < max_count:
        ...
        while q.qsize() > 0:
            e = q.get(block=False)
            result = event_handler(e, result)
            if df_handle_count <= 10000:
                df_handle_count += 1
            else:
