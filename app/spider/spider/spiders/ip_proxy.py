import scrapy
import datetime
from app.models.ip_proxy import IPProxy
    ...
    def parse(cls, response):
        try:
            ...
            for proxy in base:
                ...
                temp = IPProxy(ip=ip, port=port, schema=schema, original='无忧代理')
                temp.created_time = datetime.datetime.now().strftime('%Y-%m-%d %H:%M:%S')
                cls.proxy_list.append(temp)
