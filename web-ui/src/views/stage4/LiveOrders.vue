          ...
          <table class="data-table">
            ...
            <tbody>
              <tr v-for="record in orders" :key="record.uuid">
                ...
                <td>{{ record.volume }}</td>
                <td>{{ record.transaction_volume || 0 }}</td>
                <td>{{ record.limit_price?.toFixed(2) || '-' }}</td>
                ...
                <td>
                  <button v-if="record.status === 0" class="btn-link btn-link-danger" @click="cancelOrder(record.uuid)">
                    撤单
                  </button>
                </td>
              </tr>
