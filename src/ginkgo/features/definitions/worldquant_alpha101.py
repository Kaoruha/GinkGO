...
class WorldQuantAlpha101(BaseDefinition):
    ...
    EXPRESSIONS = {
        # 经典Alpha因子 (前20个)
        "ALPHA_001": "CS_Rank(Ts_ArgMax(SignedPower(If($returns < 0, Std($returns, 20), $close), 2), 5)) - 0.5",
        "ALPHA_002": "(-1 * Correlation(CS_Rank($delta_close), CS_Rank($volume), 6))",
        "ALPHA_003": "(-1 * Correlation(CS_Rank($open), CS_Rank($volume), 10))",
        "ALPHA_004": "(-1 * Ts_Rank(CS_Rank($close), 9))",
        "ALPHA_005": "(CS_Rank($open - Ts_Mean($vwap, 10)) * (-1 * Abs(CS_Rank($close - $vwap))))",
        "ALPHA_006": "(-1 * Correlation($open, $volume, 10))",
        "ALPHA_007": "If(Adv20($volume) < $volume, (-1 * Ts_Rank(Abs($delta_close), 60)) * Sign($delta_close), -1)",
        "ALPHA_008": "(-1 * CS_Rank(Ts_Rank($open, 5) + Ts_Rank($returns, 5)))",
        "ALPHA_009": "If(0 < Ts_Min($delta_close, 5), $delta_close, If(Ts_Max($delta_close, 5) < 0, $delta_close, (-1 * $delta_close)))",
        "ALPHA_010": "CS_Rank(If(Ts_Min($delta_close, 4) > 0, $delta_close, If(Ts_Max($delta_close, 4) < 0, $delta_close, (-1 * $delta_close))))",
        
        "ALPHA_011": "((CS_Rank(Ts_Max($vwap - $close, 3)) + CS_Rank(Ts_Min($vwap - $close, 3))) * CS_Rank($delta_volume))",
        "ALPHA_012": "(Sign($delta_volume) * (-1 * $delta_close))",
        "ALPHA_013": "(-1 * CS_Rank(Covariance(CS_Rank($close), CS_Rank($volume), 5)))",
        "ALPHA_014": "((-1 * CS_Rank($delta_returns)) * Correlation($open, $volume, 10))",
        "ALPHA_015": "(-1 * Sum(CS_Rank(Correlation(CS_Rank($high), CS_Rank($volume), 3)), 3))",
        "ALPHA_016": "(-1 * CS_Rank(Covariance(CS_Rank($high), CS_Rank($volume), 5)))",
        "ALPHA_017": "(((-1 * CS_Rank(Ts_Rank($close, 10))) * CS_Rank($delta_close / $close)) * CS_Rank(Ts_Rank($volume, 5)))",
        "ALPHA_018": "(-1 * CS_Rank((Std($abs_returns, 5) + $delta_close + Correlation($close, $volume, 2))))",
        "ALPHA_019": "((-1 * Sign($delta_close + $delta_open)) * (1 + CS_Rank(1 + Sum($returns, 250))))",
        ...
        "ALPHA_025": "CS_Rank(((-1 * $returns) * Adv20($volume) * $vwap * ($high - $close)))",
        
        "ALPHA_030": "(1.0 - CS_Rank((Sign($delta_close) + Sign(Delay($delta_close, 1)) + Sign(Delay($delta_close, 2)))))",
        "ALPHA_031": "(CS_Rank(CS_Rank(CS_Rank($decay_linear))) + CS_Rank((-1 * CS_Rank($delta_close))))",
        "ALPHA_032": "(Scale(Sum(Correlation($high, $volume, 3), 10)) / 3)",
        "ALPHA_033": "CS_Rank((-1 * Ts_Min($low, 5)) + Delay(Ts_Min($low, 5), 5))",
        "ALPHA_034": "CS_Rank(((1 - CS_Rank((Std($returns, 2) / Std($returns, 5)))) + (1 - CS_Rank($delta_close))))",
        
        # 高复杂度Alpha因子 (51-80)
        "ALPHA_051": "(((Delay($close, 20) - Delay($close, 10)) / 10) - (($close - Delay($close, 10)) / 10))",
        "ALPHA_052": "((((-1 * Ts_Min($low, 5)) + Delay(Ts_Min($low, 5), 5)) * CS_Rank(((Sum($returns, 240) - Sum($returns, 20)) / 220))) * Ts_Rank($volume, 5))",
        "ALPHA_053": "(-1 * $delta_close * Correlation($close, $volume, 10))",
        "ALPHA_054": "((-1 * (($low - $close) * ($open**5))) / (($low - $high) * ($close**5)))",
        "ALPHA_055": "(-1 * Correlation(CS_Rank($close - Ts_Min($low, 12)), CS_Rank(Adv20($volume)), 6))",
        
        "ALPHA_060": "(0 - (1 * (($close - $low) - ($high - $close)) / ($high - $low) * $volume))",
        "ALPHA_061": "(CS_Rank($vwap - Ts_Min($vwap, 16)) < CS_Rank(Correlation($vwap, Adv180($volume), 18)))",
        "ALPHA_062": "((CS_Rank(Correlation($vwap, Sum(Adv20($volume), 22), 10)) < CS_Rank(((CS_Rank($open) + CS_Rank($open)) < (CS_Rank(($high + $low)) / 2)))) * -1)",
        "ALPHA_063": "((CS_Rank($decay_linear) - CS_Rank($decay_linear**$delta_volume)) / CS_Rank($decay_linear**$delta_volume))",
        "ALPHA_064": "((CS_Rank(Correlation(Sum($close, 10), Sum(Adv60($volume), 10), 10)) < CS_Rank((-1 * Std($close, 10)))) * -1)",
        
        # 终极复杂Alpha因子 (81-101)
        "ALPHA_081": "((CS_Rank(Log(Product(CS_Rank((CS_Rank(Correlation($vwap, Sum(Adv10($volume), 50), 8))**4)), 15))) < CS_Rank(Correlation(CS_Rank($vwap), CS_Rank($volume), 5))) * -1)",
        "ALPHA_082": "((Min(CS_Rank($decay_linear), Ts_Rank($decay_linear, 20)) + Ts_Rank((-1 * $delta_close), 8)) < Ts_Rank(((($high + $low) / 2) * 0.2 + $vwap * 0.8), 15))",
        "ALPHA_083": "((CS_Rank(Delay(((($high - $low) / (Sum($close, 5) / 5)) * $volume), 2)) * CS_Rank(CS_Rank($volume))) / (((($high - $low) / (Sum($close, 5) / 5)) * $volume) / Adv20($volume)))",
        "ALPHA_084": "SignedPower(Ts_Rank($vwap - Ts_Max($vwap, 15), 21), $delta_close)",
        "ALPHA_085": "(CS_Rank(Correlation(($high * 0.876703 + $close * 0.123297), Adv30($volume), 10))**CS_Rank(Correlation(Ts_Rank(($high + $low) / 2, 4), Ts_Rank($volume, 10), 7)))",
        
        "ALPHA_090": "((CS_Rank((($close - Ts_Max($close, 5)) / Ts_Max($close, 5))) * CS_Rank(Correlation(IndNeutralize($adv40, $sector), $low, 5))) * -1)",
        "ALPHA_091": "((Ts_Rank(Delay($close, 5), 20) * CS_Rank(((1 - CS_Rank(($high * $low))) * (1 + CS_Rank($volume))))) * -1)",
        "ALPHA_092": "Min(Ts_Rank($decay_linear, 30), Ts_Rank($decay_linear, 4))",
        "ALPHA_093": "(Ts_Rank($decay_linear, 73) / Ts_Rank($decay_linear, 11))",
        "ALPHA_094": "((CS_Rank(($vwap - Ts_Min($vwap, 11)))**Ts_Rank(Correlation(Ts_Rank($vwap, 20), Ts_Rank(Adv60($volume), 4), 18), 3)) * -1)",
        
        "ALPHA_095": "(CS_Rank(($open - Ts_Min($open, 12))) < Ts_Rank(CS_Rank(Correlation(Sum(($high + $low) / 2, 19), Sum(Adv40($volume), 19), 13)), 5))",
        "ALPHA_096": "((Correlation(CS_Rank($vwap), CS_Rank($volume), 4) < CS_Rank(Ts_Rank(Ts_ArgMax($close, 7), 5))) * -1)",
        "ALPHA_097": "((CS_Rank($decay_linear) - CS_Rank(Correlation(IndNeutralize($adv20, $sector), $low, 7))) / CS_Rank(Ts_Rank($delta_close, 14)))",
        "ALPHA_098": "(CS_Rank($decay_linear)**CS_Rank(Correlation(Sum($adv5, 26), Sum($adv15, 26), 4)))",
        "ALPHA_099": "((CS_Rank(Correlation(Sum($high, 5), Sum($volume, 5), 5)) < CS_Rank(Ts_Rank(Ts_ArgMin($high, 30), 4))) * -1)",
        "ALPHA_100": "(0 - (1 * (((1.5 * Scale(IndNeutralize($close, $sector))) - Scale(IndNeutralize(Delay($close, 1), $sector))) * (($volume / Adv20($volume)) / 2))))",
        "ALPHA_101": "((Correlation(CS_Rank($close), CS_Rank(Adv20($volume)), 9) < CS_Rank(((9.5 - Ts_Rank(Ts_ArgMax(DecayLinear($high, 2), 4), 14)) / 9))) * -1)",
        
        # 基于排名的衍生因子
        "RANK_VOLUME_PRICE": "CS_Rank($volume) / CS_Rank($close)",
        "RANK_MOMENTUM_VOLATILITY": "CS_Rank($momentum_20) / CS_Rank($volatility_20)",
        "CROSS_RANK_CORRELATION": "Correlation(CS_Rank($close), CS_Rank($volume), 10)",
        "RANK_MEAN_REVERSION": "CS_Rank($close) - CS_Rank(Ts_Mean($close, 20))",
        ...
        # 复合排名因子
        "COMPOSITE_RANK_1": "CS_Rank(Ts_Rank($close, 10) + Ts_Rank($volume, 10))",
        "COMPOSITE_RANK_2": "CS_Rank(Correlation($close, $volume, 5) + $momentum_10)",
    }
