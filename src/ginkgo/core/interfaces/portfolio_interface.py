...
class BasePortfolio(ABC):
    ...
    def risk_managements(self) -> List[BaseRiskManagement]:
        """Compatibility alias for the historical risk_managements name."""
        return self.risk_managers
    ...
    def add_risk_manager(self, risk_mgmt: BaseRiskManagement) -> None:
        """添加风险管理器"""
        if risk_mgmt not in self._risk_managers:
            self._risk_managers.append(risk_mgmt)

    def add_risk_managements(self, risk_mgmt: BaseRiskManagement) -> None:
        """Compatibility alias for the historical add_risk_managements name."""
        self.add_risk_manager(risk_mgmt)
    ...
    def update_position(self, code: str, quantity: float, price: float) -> None:
        ...
        if code not in self._positions:
            self._positions[code] = {"quantity": 0, "avg_price": 0, "market_value": 0, "unrealized_pnl": 0}
    ...
    def get_position(self, code: str) -> Dict[str, Any]:
        """获取单只股票持仓"""
        return self._positions.get(code, {"quantity": 0, "avg_price": 0, "market_value": 0, "unrealized_pnl": 0})
