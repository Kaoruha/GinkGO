...
运行记录模型：记录每次执行的详细信息，支持运行历史追踪和统计分析。
...
class MRunRecord(MMysqlBase, MBacktestRecordBase):
    """
    运行记录模型：记录每次执行的详细信息
    ...
    def start_run(self, config_snapshot: str = "{}", environment_info: str = "{}") -> None:
        """
        标记运行开始
    ...
    def finish_run(self, status: str = "completed", error_message: str = "", 
                   final_portfolio_value: str = "0", total_pnl: str = "0", 
                   max_drawdown: str = "0") -> None:
        """
        标记运行结束
    ...
    def __repr__(self) -> str:
        return base_repr(self, f"RunRecord[{self.status}]", 12, 80)