...
# Role: TransferCRUD资金划转CRUD继承BaseCRUD提供资金划转管理功能
...
class TransferCRUD(BaseCRUD[MTransfer]):
    ...
    def find_by_status(self, status: TRANSFERSTATUS_TYPES) -> List[MTransfer]:
        """
        Business helper: Find transfers by status.
        """
        return self.find(filters={"status": status}, order_by="timestamp", desc_order=True)

    def find_by_direction(self, direction: TRANSFERDIRECTION_TYPES) -> List[MTransfer]:
        """
