...
from ...libs import base_repr, datetime_normalize, GLOG
...
class Signal(Base):
    ...
    def is_valid(self) -> bool:
        ...
        if not self._portfolio_id:
            GLOG.WARNING(f"Signal validation failed: missing portfolio_id")
            return False
            
        if not self._engine_id:
            GLOG.WARNING(f"Signal validation failed: missing engine_id")
            return False
            
        if not self._code:
            GLOG.WARNING(f"Signal validation failed: missing code")
            return False
            
        if self._direction is None:
            GLOG.WARNING(f"Signal validation failed: missing direction")
            return False
            
        if not isinstance(self._direction, DIRECTION_TYPES):
            GLOG.WARNING(f"Signal validation failed: invalid direction type {type(self._direction)}")
            return False
            
        if self._timestamp is None:
            GLOG.WARNING(f"Signal validation failed: missing timestamp")
            return False
            
        # 检查时间戳是否合理（不能是未来时间）
        if self._timestamp > datetime.datetime.now():
            GLOG.WARNING(f"Signal validation failed: timestamp is in the future {self._timestamp}")
            return False
