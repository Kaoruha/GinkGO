...
from ginkgo.data.operations import get_stockinfos_filtered, get_bars_page_filtered
...
class MomentumSelector(BaseSelector):
    ...
    def pick(self, time: any = None, *args, **kwargs) -> list[str]:
        ...
        df = get_stockinfos_filtered()
        df = df[:50]
        for i, r in df.iterrows():
            code = r["code"]
            name = r["code_name"]
            self.log("DEBUG", f"Pick {code} from {start_date} to {end_date}")
            df = get_bars_page_filtered(code=code, start_date=start_date, end_date=end_date, as_dataframe=True)
            if df.shape[0] == 0:
                continue
