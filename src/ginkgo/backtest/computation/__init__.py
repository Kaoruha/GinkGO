...
from .technical.base_indicator import BaseIndicator
...
def _warn_deprecated_import(old_module: str):
    """发出弃用警告的辅助函数"""
    warnings.warn(
        f"从 '{old_module}' 导入已弃用。请使用 'from ginkgo.backtest.computation import ...' "
        f"此兼容性层将在未来版本中移除。",
        DeprecationWarning,
        stacklevel=3
    )
...
class _CompatibilityLayer:
    ...
    def indicators_import():
        _warn_deprecated_import("ginkgo.backtest.indicators")
        return True
    
    @staticmethod  
    def indices_import():
        _warn_deprecated_import("ginkgo.backtest.indices")
        return True
