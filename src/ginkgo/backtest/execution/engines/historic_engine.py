...
from ginkgo.backtest.execution.events import EventNextPhase
...
class HistoricEngine(EventEngine):
    ...
    def next_phase(self, *args, **kwargs) -> None:
        if self.now is None and self.start_date is None:
            self.log("CRITICAL", f"{self.name} Fatal error: No start_date or current time configured")
            self.stop()
            sys.exit(0)
        ...
        try:
            if self.now >= self.end_date:
                ...
                if self._empty_count >= self.max_waits:
                    self.log("INFO", f"Backtest completed successfully. End time: {self.end_date}")
                    self.stop()
                    sys.exit(0)
                return
            ...
            pdb.set_trace()  # 修复：添加括号
            print(e)

        if self.matchmaking is None:
            self.log("CRITICAL", f"{self.name} Fatal error: No matchmaking configured")
            self.stop()
            sys.exit(0)
        self.matchmaking.on_time_goes_by(self.now)

        if self.datafeeder is None:
            self.log("CRITICAL", f"{self.name} Fatal error: No datafeeder configured")
            self.stop()
            sys.exit(0)
        self.datafeeder.on_time_goes_by(self.now)

        if len(self.portfolios) == 0:
            self.log("CRITICAL", f"{self.name} Fatal error: No portfolios configured")
            self.stop()
            sys.exit(0)
        else:
