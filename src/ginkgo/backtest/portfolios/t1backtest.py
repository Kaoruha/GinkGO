...
class PortfolioT1Backtest(BasePortfolio):
    ...
    def on_signal(self, event: EventSignalGeneration):
        ...
        """
        self.log("INFO", f"Got a new Signal about {event.code}. {self.now}")
        self.log(
        ...
        if order is None:
            ...
            return
        self.log("INFO", f"Generate an ORDER by sizer. {self.now}")
    ...
    def on_price_received(self, event: EventPriceUpdate):
        # Check Everything.
        self.log("INFO", f"Got new price. {self.now}")
        if not self.is_all_set():
            return
    ...
    def deal_long_filled(self, event: EventOrderFilled, *args, **kwargs):
        if self.frozen < event.frozen:
            ...
            return
        if event.remain < 0:
            sefl.log("CRITICAL", f"Order can not remain under 0.")
            return
        self.unfreeze(event.frozen)
