...
class PositionRatioRisk(BaseRiskManagement):
    ...
    def cal(self, portfolio_info: Dict, order: Order) -> Order:
        ...
        """
        self.log("DEBUG", f"PositionRatioRisk: Processing order - Code: {order.code}, Direction: {order.direction}, Volume: {order.volume}, Type: {order.order_type}, Limit Price: {order.limit_price}")
        
        if order.direction != DIRECTION_TYPES.LONG:
            # 卖出订单不受持仓比例限制
            self.log("DEBUG", f"PositionRatioRisk: Sell order for {order.code} passed through without restriction")
            return order
        ...
        self.log("DEBUG", f"PositionRatioRisk: Portfolio total worth: {total_worth}")
        ...
        for code, position in positions.items():
            if position and position.volume > 0:
                ...
                position_count += 1
                self.log("DEBUG", f"PositionRatioRisk: Position {code} - Volume: {position.volume}, Price: {position.price}, Value: {position_value}")
                if code == order.code:
                    current_position_value = position_value

        # 计算当前总持仓比例
        current_total_ratio = total_position_value / total_worth
        self.log("DEBUG", f"PositionRatioRisk: Current positions - Count: {position_count}, Total value: {total_position_value}, Total ratio: {float(current_total_ratio):.2%}")
        self.log("DEBUG", f"PositionRatioRisk: Current position value for {order.code}: {current_position_value}")
        ...
        self.log("DEBUG", f"PositionRatioRisk: Order {order.code}, type: {order.order_type}, limit_price: {order.limit_price}, execution_price: {execution_price}")
            
        if execution_price <= 0:
            self.log("WARN", f"PositionRatioRisk: Invalid execution price {execution_price} for {order.code}, order type: {order.order_type}, limit_price: {order.limit_price}")
            return None
        ...
        self.log("DEBUG", f"PositionRatioRisk: Order value calculation - Volume: {order.volume}, Price: {execution_price}, Order value: {order_value}")
        ...
        self.log("DEBUG", f"PositionRatioRisk: Single position ratio check for {order.code}")
        self.log("DEBUG", f"  Current ratio: {float(current_position_ratio):.2%}, Expected ratio: {float(expected_position_ratio):.2%}, Limit: {float(self._max_position_ratio):.2%}")
        ...
        if expected_position_ratio > self._max_position_ratio:
            ...
            self.log("DEBUG", f"PositionRatioRisk: Single position limit exceeded - Max allowed position value: {max_allowed_position_value}, Max allowed order value: {max_allowed_order_value}")
            
            if max_allowed_order_value <= 0:
                self.log("INFO", f"PositionRatioRisk: Single position ratio limit reached for {order.code} - current ratio: {float(current_position_ratio):.2%}, limit: {float(self._max_position_ratio):.2%}")
                return None
            ...
            if adjusted_volume < to_decimal(order.volume):
                self.log("INFO", f"PositionRatioRisk: Adjusting order volume for single position ratio - {order.code}: {original_volume} → {int(adjusted_volume)} (from {float(expected_position_ratio):.2%} to {float(self._max_position_ratio):.2%})")
        ...
        self.log("DEBUG", f"PositionRatioRisk: Total position ratio check")
        self.log("DEBUG", f"  Current total ratio: {float(current_total_ratio):.2%}, Expected total ratio: {float(expected_total_ratio):.2%}, Limit: {float(self._max_total_position_ratio):.2%}")
        ...
        if expected_total_ratio > self._max_total_position_ratio:
            ...
            self.log("DEBUG", f"PositionRatioRisk: Total position limit exceeded - Max allowed total value: {max_allowed_total_value}, Max allowed order value: {max_allowed_order_value}")
            
            if max_allowed_order_value <= 0:
                self.log("INFO", f"PositionRatioRisk: Total position ratio limit reached - current ratio: {float(current_total_ratio):.2%}, limit: {float(self._max_total_position_ratio):.2%}")
                return None
            ...
            original_volume = order.volume
            if adjusted_volume < to_decimal(order.volume):
                self.log("INFO", f"PositionRatioRisk: Adjusting order volume for total position ratio - {order.code}: {original_volume} → {int(adjusted_volume)} (from {float(expected_total_ratio):.2%} to {float(self._max_total_position_ratio):.2%})")
                order.volume = int(adjusted_volume)
                order.frozen = adjusted_volume * execution_price
                order.remain = order.frozen
        ...
        # 如果调整后的订单量为0，则拒绝订单
        if order.volume <= 0:
            self.log("INFO", f"PositionRatioRisk: Order rejected due to position ratio limits - final volume: {order.volume}")
            return None
        ...
        self.log("DEBUG", f"PositionRatioRisk: Order approved - Final volume: {order.volume}, Final position ratio: {float(final_position_ratio):.2%}, Final total ratio: {float(final_total_ratio):.2%}")
    ...
    def _get_current_price(self, portfolio_info: Dict, code: str) -> Decimal:
        ...
        if self._data_feeder is not None:
            try:
                ...
                self.log("WARN", f"PositionRatioRisk: Failed to get current price from feeder: {e}")
        
        # 如果无法获取，记录警告并返回0
        self.log("WARN", f"PositionRatioRisk: Unable to get current price for {code}")
        return to_decimal(0)
    ...
    def generate_signals(self, portfolio_info: Dict, event) -> List[Signal]:
        ...
        self.log("DEBUG", f"PositionRatioRisk: Monitoring position ratios - Portfolio worth: {total_worth}")
        ...
        for code, position in positions.items():
            if position and position.volume > 0:
                ...
                self.log("DEBUG", f"PositionRatioRisk: Position {code} - Volume: {position.volume}, Price: {position.price}, Value: {position_value}, Ratio: {float(position_ratio):.2%}")
        
        # 记录总持仓情况
        total_ratio = total_position_value / total_worth if total_worth > 0 else 0
        self.log("DEBUG", f"PositionRatioRisk: Total positions - Count: {len(position_details)}, Total value: {total_position_value}, Total ratio: {float(total_ratio):.2%}")
        ...
        for pos_detail in position_details:
            ...
            if position_ratio > warning_ratio:
                ...
                self.log("INFO", f"  Current ratio: {position_ratio_float:.2%}, Warning threshold: {warning_ratio_float:.2%}, Max allowed: {max_ratio_float:.2%}")
        ...
        if signals:
            ...
            self.log("DEBUG", f"PositionRatioRisk: No position ratio warnings triggered")
