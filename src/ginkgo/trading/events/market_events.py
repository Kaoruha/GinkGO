# Upstream: 时间推进控制器(时钟节拍控制)、引擎(市场状态监控)、策略(K线结束触发计算)、系统(日终处理)
# Downstream: EventBase(事件基类)、EVENT_TYPES枚举(事件类型)
# Role: 市场事件模块定义开市/收市/暂停/恢复等事件类支持交易系统功能用于交易系统事件管理和状态控制提供市场状态追踪
