...
class PortfolioLive(PortfolioBase):
    ...
    def log(self, level: str, msg: str, *args, **kwargs) -> None:
        ...
        # 使用GLOG记录
        final_msg = f"{self.name}: {formatted_msg}"
        if level_up == "DEBUG":
            GLOG.DEBUG(final_msg)
        elif level_up == "INFO":
            GLOG.INFO(final_msg)
        elif level_up == "WARNING" or level_up == "WARN":
            GLOG.WARN(final_msg)
        elif level_up == "ERROR":
            GLOG.ERROR(final_msg)
        elif level_up == "CRITICAL":
            GLOG.CRITICAL(final_msg)
        else:
            GLOG.INFO(final_msg)
    ...
    def _process_signal(self, signal: Signal, event_timestamp=None):
        ...
        """
        GLOG.INFO(f"{self.name} processing signal: {signal.direction} for {signal.code}")
        ...
        if order is None:
            GLOG.INFO(f"No order generated by sizer for signal {signal.code}")
            return
        ...
        for risk_manager in self.risk_managers:
            ...
            if order_adjusted is None:
                GLOG.INFO(f"Order for {signal.code} blocked by risk manager {risk_manager.name}")
                break
        ...
        if order_adjusted.volume == 0:
            GLOG.INFO(f"Order for {signal.code} has zero volume")
            return
        ...
        try:
            ...
            for signal in all_signals:
                ...
                try:
                    ...
                    GLOG.ERROR(f"Failed to process signal for {signal.code}: {e}")

        except Exception as e:
            GLOG.ERROR(f"on_price_update failed for {event.code}: {e}")
    ...
    def on_order_cancel_ack(self, event: EventOrderCancelAck) -> None:
        ...
        GLOG.WARN(f"Dealing with CANCELED ORDER. {self.get_time_provider().now()}")
        ...
        if event.direction == DIRECTION_TYPES.LONG:
            ...
            GLOG.INFO(f"Short order {event.code} CANCELED. Released frozen volume {getattr(event, 'cancelled_quantity', 0)}")
    ...
    def sync_state_to_db(self) -> bool:
        ...
        try:
            ...
            GLOG.ERROR(f"sync_state_to_db failed: {e}")
            return False
    ...
    def _set_status(self, status: PORTFOLIO_RUNSTATE_TYPES):
        ...
        with self._status_lock:
            ...
            GLOG.INFO(f"Status changed: {old_status.value} -> {status.value}")
    ...
    def _sync_status_to_redis(self, status: PORTFOLIO_RUNSTATE_TYPES):
        ...
        try:
            ...
            GLOG.ERROR(f"Failed to sync status to Redis: {e}")
    ...
    def graceful_reload(self, timeout: int = 30) -> bool:
        ...
        """
        try:
            GLOG.INFO(f"Starting graceful reload (timeout={timeout}s)")
            ...
            GLOG.INFO("Waiting for existing messages to be processed...")
            ...
            # 步骤 4: 优雅关闭（清理资源）
            # ========================================
            GLOG.INFO("Cleaning up resources...")
            # TODO: 调用清理方法（如果需要）
            ...
            # 步骤 6: 加载新配置
            # ========================================
            GLOG.INFO("Loading new configuration from database...")
            new_config = self._load_config_from_db()

            if new_config is None:
                GLOG.ERROR("Failed to load new configuration")
            ...
            if self._config_version and new_config.get('version') == self._config_version:
                GLOG.WARN("Configuration version unchanged, skipping reload")
            ...
            # 步骤 7: 重新初始化（应用新配置）
            # ========================================
            GLOG.INFO("Reinitializing with new configuration...")
            ...
            GLOG.INFO("Graceful reload completed successfully")
            return True

        except Exception as e:
            GLOG.ERROR(f"Graceful reload failed: {e}")
    ...
    def _load_config_from_db(self) -> Optional[Dict]:
        ...
        try:
            ...
            if not result.is_success() or not result.data:
                GLOG.ERROR(f"Portfolio {self.portfolio_id} not found in database")
                return None
            ...
            GLOG.INFO(f"Configuration loaded: version={config['version']}")
            return config

        except Exception as e:
            GLOG.ERROR(f"Failed to load configuration from database: {e}")
            return None
    ...
    def _reinitialize(self, new_config: Dict):
        ...
        try:
            ...
            GLOG.INFO(f"Reinitialized with config: {new_config.get('version')}")

        except Exception as e:
            GLOG.ERROR(f"Failed to reinitialize: {e}")
            raise
    ...
    def buffer_event(self, event: Any):
        ...
        if len(self._event_buffer) >= self._max_buffer_size:
            GLOG.WARN(f"Event buffer full ({self._max_buffer_size}), dropping oldest event")
            self._event_buffer.pop(0)  # 移除最旧的事件

        self._event_buffer.append(event)
        GLOG.DEBUG(f"Event buffered (buffer size: {len(self._event_buffer)})")
    ...
    def clear_buffer(self):
        ...
        GLOG.DEBUG("Event buffer cleared")
