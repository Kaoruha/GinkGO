...
class TimeRelated:
    def __init__(self, timestamp: any = None, init_time: any = None, *args, **kwargs) -> None:
        # 对于Strategy等回测组件，允许timestamp为None，等待引擎时间注入
        if timestamp is not None:
            # 验证timestamp格式是否有效
            normalized_timestamp = datetime_normalize(timestamp)
            if normalized_timestamp is None:
                raise ValueError(f"Invalid timestamp format: {timestamp}")
            self._timestamp: datetime.datetime = normalized_timestamp
        else:
            # timestamp为None时，等待后续时间注入
        ...
        self._init_time: datetime.datetime = datetime_normalize(init_time) if init_time is not None else current_time
    ...
    def timestamp(self) -> datetime.datetime:
        """
        The timestamp when this entity's data was created/occurred.
        """
    ...
    def timestamp(self, value: any) -> None:
        """
        Set the timestamp for this entity.
        """
    ...
    def set_time_provider(self, time_provider: Any) -> None:
        """
        设置时间提供者（ITimeAwareComponent接口）

        Args:
            time_provider: ITimeProvider实例
        """
