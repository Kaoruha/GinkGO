...
from ginkgo.enums import ENGINE_ARCHITECTURE, EXECUTION_MODE
...
class BacktestConfig:
    ...
    # 引擎配置
    engine_architecture: ENGINE_ARCHITECTURE = ENGINE_ARCHITECTURE.AUTO
    execution_mode: EXECUTION_MODE = EXECUTION_MODE.BACKTEST
    ...
    def get_engine_config(self, engine_architecture: ENGINE_ARCHITECTURE = None) -> Dict[str, Any]:
        """获取特定引擎架构的配置"""
        arch = engine_architecture or self.engine_architecture

        if arch == ENGINE_ARCHITECTURE.EVENT_DRIVEN:
            return self.event_engine_config
        elif arch == ENGINE_ARCHITECTURE.MATRIX:
            return self.matrix_engine_config
        elif arch == ENGINE_ARCHITECTURE.HYBRID:
            return self.hybrid_engine_config
        else:
            return {}

    def set_engine_config(self, engine_architecture: ENGINE_ARCHITECTURE, config: Dict[str, Any]) -> None:
        """设置特定引擎架构的配置"""
        if engine_architecture == ENGINE_ARCHITECTURE.EVENT_DRIVEN:
            self.event_engine_config.update(config)
        elif engine_architecture == ENGINE_ARCHITECTURE.MATRIX:
            self.matrix_engine_config.update(config)
        elif engine_architecture == ENGINE_ARCHITECTURE.HYBRID:
    ...
    def optimize_for_large_dataset(self) -> None:
        """为大数据集优化"""
        self.engine_architecture = ENGINE_ARCHITECTURE.MATRIX
    ...
    def optimize_for_realtime(self) -> None:
        """为实时交易优化"""
        self.engine_architecture = ENGINE_ARCHITECTURE.EVENT_DRIVEN
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        return {
            ...
            'engine_architecture': self.engine_architecture.value,
    ...
    def from_dict(cls, config_dict: Dict[str, Any]) -> 'BacktestConfig':
        """从字典创建配置对象"""
        # 转换枚举值
        if 'engine_architecture' in config_dict:
            config_dict['engine_architecture'] = ENGINE_ARCHITECTURE(config_dict['engine_architecture'])
        if 'execution_mode' in config_dict:
            config_dict['execution_mode'] = EXECUTION_MODE(config_dict['execution_mode'])
    ...
    def __str__(self) -> str:
        return f"BacktestConfig(name={self.name}, period={self.start_date}~{self.end_date}, arch={self.engine_architecture.value}, mode={self.execution_mode.value})"
...
class ConfigTemplates:
    ...
    def day_trading_config() -> BacktestConfig:
        ...
        config = BacktestConfig(
            ...
            engine_architecture=ENGINE_ARCHITECTURE.EVENT_DRIVEN,
    ...
    def swing_trading_config() -> BacktestConfig:
        ...
        config = BacktestConfig(
            ...
            engine_architecture=ENGINE_ARCHITECTURE.MATRIX,
            execution_mode=EXECUTION_MODE.BACKTEST,
    ...
    def ml_research_config() -> BacktestConfig:
        ...
        config = BacktestConfig(
            ...
            data_frequency=DataFrequency.DAILY,
            engine_architecture=ENGINE_ARCHITECTURE.AUTO,
            execution_mode=EXECUTION_MODE.BACKTEST,
