    ...
    try:
        ...
        if key:
            ...
            if key.lower() == 'debug':
                ...
                # ADR-026 client 模式四键：mode/api_host/api_port/api_tls 均可 get
                _val = {
                    'mode': GCONF.MODE,
...
def set(
    ...
    try:
        ...
        if key.lower() == 'debug':
            ...
            # ADR-026 client 模式：写 config.yml 平铺 key（_get_config 经 mtime 自动重载）
...
def list():
    ...
    try:
        ...
        table.add_row("mode", "string", "Run mode: local | client (ADR-026)")
