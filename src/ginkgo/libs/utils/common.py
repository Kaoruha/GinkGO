...
def cache_with_expiration(func=None, *, expiration_seconds=60, max_cache_size=64):  # 默认缓存时长60秒，默认上限64条
    # ATTENTION expiration secconds too large will cause the mem leak.
    # max_cache_size 控制每个被装饰函数独立缓存的最大条目数（超出按 LRU 驱逐）
    # cache_data 在每个被装饰函数的闭包内独立创建，避免跨函数互相驱逐（#3863）
    if func is None:

        def decorator(f):
            cache_data = OrderedDict()  # 每个被装饰函数独立缓存实例
            @wraps(f)
            def wrapper(*args, **kwargs):
                ...
                if cache_key in cache_data:
                    ...
                    if cached_value is not None:
                        ...
                        if time.time() - timestamp < expiration_seconds:
                            ...
                            cache_data.move_to_end(cache_key)  # LRU：命中时提升为最近使用
                            return result
                        else:
        ...
        def wrapper(*args, **kwargs):
            ...
            # 存入缓存并记录时间戳
            cache_data[cache_key] = (result, time.time())
            # LRU 驱逐：超出 max_cache_size 时弹最久未访问的条目（防内存泄漏 #3863）
            if len(cache_data) > max_cache_size:
                cache_data.popitem(last=False)
            return result
