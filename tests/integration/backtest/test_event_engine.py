...
@pytest.fixture
def event_engine_with_time(event_engine):
    """Create an EventEngine for tests that previously needed time setup."""
    return event_engine
...
class TestTimerHandlers:
    ...
    def test_engine_timer_process(self, event_engine):
        ...
        engine.enable_timer()  # Must enable timer before start
        engine.start()
        time.sleep(0.5)
        engine.stop()
...
class TestEventEngineAttributes:
    ...
    def test_queue_attribute(self, event_engine_with_time):
        ...
        assert hasattr(event_engine_with_time, "_event_queue")
        assert isinstance(event_engine_with_time._event_queue, Queue)
...
class TestConcurrentProcessing:
    ...
    def test_concurrent_event_handling(self, event_engine):
        ...
        while event_engine.todo_count > 0:
            try:
                event_obj = event_engine._event_queue.get_nowait()
