...
class TestAdjustfactorServiceCRUD:
    ...
    def test_add_various_factors(self, adjustfactor_service, unique_code, fore_factor, back_factor, adj_factor):
        ...
        assert result.foreadjustfactor == Decimal(str(fore_factor))
        assert result.backadjustfactor == Decimal(str(back_factor))
        assert result.adjustfactor == Decimal(str(adj_factor))
