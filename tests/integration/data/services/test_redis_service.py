...
class TestRedisCacheOperations:
    ...
    def test_cache_with_expiration(self, redis_service, test_prefix):
        ...
        # 设置1秒过期
        redis_service.set_cache(key, value, expire_seconds=1)
...
@pytest.mark.unit
@pytest.mark.db_cleanup
@pytest.mark.skip(reason="RedisService task status API mismatch: set_task_status/delete_task_status signatures changed")
...
@pytest.mark.unit
@pytest.mark.skip(reason="RedisService get_redis_info returns Dict, not ServiceResult; health_check not implemented")
...
@pytest.mark.unit
@pytest.mark.db_cleanup
@pytest.mark.skip(reason="RedisService find_keys/delete_keys method signatures changed")
...
@pytest.mark.unit
@pytest.mark.skip(reason="RedisService validate_connection method not implemented")
...
class TestRedisServiceIntegration:
    ...
    def test_cache_workflow(self, redis_service):
        ...
        # 批量删除缓存
        for i in range(5):
            redis_service.delete_cache(f"{prefix}_key_{i}")

    @pytest.mark.skip(reason="RedisService task status API changed")
