...
class TestSignalTrackerCRUDBusinessLogic:
    ...
    def test_signal_execution_quality_analysis(self):
        ...
        try:
            ...
            # 分析执行偏差（仅分析已执行的信号）
            executed_trackers = [t for t in all_trackers if t.is_executed() and hasattr(t, 'price_deviation') and t.price_deviation is not None]

            if executed_trackers:
                price_deviations = [float(t.price_deviation) for t in executed_trackers]
                volume_deviations = [float(t.volume_deviation) for t in executed_trackers if hasattr(t, 'volume_deviation') and t.volume_deviation is not None]
                time_delays = [t.time_delay_seconds for t in executed_trackers if hasattr(t, 'time_delay_seconds') and t.time_delay_seconds is not None]
    ...
    def test_execution_mode_efficiency(self):
        ...
        try:
            ...
            for tracker in all_trackers:
                ...
                if tracker.is_executed() and hasattr(tracker, 'time_delay_seconds') and tracker.time_delay_seconds is not None:
...
class TestSignalTrackerCRUDDelete:
    ...
    def test_delete_signal_tracker_by_execution_mode(self):
        ...
        print("→ 准备测试数据...")
        test_modes = [
            ("mode_sim_test", EXECUTION_MODE.BACKTEST, ACCOUNT_TYPE.PAPER),
            ("mode_sim_test_2", EXECUTION_MODE.BACKTEST, ACCOUNT_TYPE.PAPER),
        ...
        try:
            # 删除模拟模式的数据
            print("\n→ 执行模式删除操作...")
            before_count = len(signal_tracker_crud.find(filters={"execution_mode": EXECUTION_MODE.BACKTEST}))
            print(f"✓ 删除前模拟模式数据量: {before_count}")

            signal_tracker_crud.remove(filters={"execution_mode": EXECUTION_MODE.BACKTEST})
            print("✓ 执行模式删除操作完成")

            # 验证删除结果
            after_count = len(signal_tracker_crud.find(filters={"execution_mode": EXECUTION_MODE.BACKTEST}))
