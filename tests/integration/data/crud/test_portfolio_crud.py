...
class TestPortfolioCRUDQuery:
    ...
    def test_model_list_conversions(self):
        """测试list的to_dataframe和identity转换功能"""
        import pandas as pd

        print("\n" + "="*60)
        print("开始测试: Portfolio list转换功能")
        print("="*60)
...
class TestPortfolioCRUDBusinessLogic:
    ...
    def test_portfolio_data_integrity(self):
        ...
        try:
            ...
            # name不能为空 - 当前MPortfolio模型层不强制校验name
            # 名称验证在Service层(EngineService.add)处理
            print("→ 跳过模型层name验证（由Service层负责）")
...
class TestPortfolioCRUDEnumValidation:
    ...
    def test_comprehensive_enum_validation(self):
        ...
        assert len(model_list) >= len(enum_combinations), f"list应该包含至少{len(enum_combinations)}条测试投资组合"
