...
from ginkgo.trading.events.time_advance import EventTimeAdvance, EventTimeAdvance as EventTime
...
class TestBacktestModeManagerCore:
    ...
    def test_backtest_determinism_verification(self):
        ...
        while processed1 < event_count:
            event = safe_get_event(engine1, timeout=0.1)
            if event:
        ...
        while processed2 < event_count:
            event = safe_get_event(engine2, timeout=0.1)
            if event:
...
class TestEndToEnd:
    ...
    @pytest.mark.skip(reason="测试依赖引擎启动后的事件处理循环，手动消费队列无法触发handler")
    ...
    def test_system_integration_workflow(self):
        ...
        data_index = actual_stages.index('data_processing_phase')
        assert init_index < data_index, "初始化应在数据处理之前"

        # 策略执行应在数据之后
        strategy_index = actual_stages.index('strategy_execution_phase')
        assert data_index < strategy_index, "数据处理应在策略执行之前"

        # 风控管理应在策略执行之后
        risk_index = actual_stages.index('risk_management_phase')
