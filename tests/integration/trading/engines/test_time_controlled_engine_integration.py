    ...
    def test_complete_backtest_integration(self):
        ...
        for day_data in backtest_data:
            ...
            for event_data in day_data['events']:
                ...
                price_event = EventPriceUpdate(payload=tick, timestamp=event_datetime)
    ...
    def test_time_provider_global_integration(self):
        ...
        for time_point in test_time_points:
            ...
            for i, engine in enumerate(engines):
                ...
                price_event = EventPriceUpdate(payload=tick, timestamp=time_point)
                engine.put(price_event)
...
class TestPerformanceAndStress:
    ...
    @pytest.mark.skip(reason="Flaky: event count varies due to async queue contention")
    def test_high_frequency_event_processing(self):
        ...
        for i in range(event_count):
            ...
            price_event = EventPriceUpdate(payload=tick, timestamp=new_time)
            engine.put(price_event)
    ...
    @pytest.mark.skip(reason="内存测试超时，需优化")
        ...
        try:
            ...
            for batch in range(event_batches):
                ...
                for i in range(100):
                    ...
                    price_event = EventPriceUpdate(payload=tick, timestamp=event_time)
                    engine.put(price_event)
    ...
    def test_concurrent_handler_scalability(self):
        """测试并发处理器扩展性"""
        # 测试注册不同数量的处理器对引擎正常功能的影响
        handler_counts = [1, 5, 20]
        for count in handler_counts:
            engine = TimeControlledEventEngine(
                name=f"ScalabilityEngine_{count}",
                mode=EXECUTION_MODE.BACKTEST
            )

            # 注册指定数量的处理器（使用真实函数，避免Mock的__name__问题）
            handlers = []
            for i in range(count):
                def make_handler(idx):
                    def handler(event):
                        pass
                    handler.__name__ = f"test_handler_{idx}"
                    return handler
                h = make_handler(i)
                engine.register(EVENT_TYPES.PRICEUPDATE, h)
                handlers.append(h)

            # 验证处理器被正确注册
            assert engine.handler_count >= count, f"应注册至少{count}个处理器"
            ...
            try:
                tick = Tick(
                    ...
                    volume=1000,
                    direction=TICKDIRECTION_TYPES.NEUTRAL,
                    timestamp=dt(2023, 1, 1, 10, 0, 0, tzinfo=timezone.utc)
                )
                price_event = EventPriceUpdate(payload=tick, timestamp=dt(2023, 1, 1, 10, 0, 0, tzinfo=timezone.utc))
                engine.put(price_event)
    ...
    def test_time_advancement_performance(self):
        """测试时间推进性能"""
        # 只测试time_provider的时间推进性能，不需要启动引擎线程
        engine = TimeControlledEventEngine(
            name="TimeAdvPerfEngine",
            mode=EXECUTION_MODE.BACKTEST
        )
        ...
        avg_per_advance = elapsed / iterations

        # 验证每次推进平均耗时在合理范围内（<10ms）
        assert avg_per_advance < 0.01, f"平均每次推进耗时{avg_per_advance*1000:.3f}ms，应小于10ms"
        # 验证时间确实被推进了
        assert engine._time_provider.now() >= start_time + timedelta(hours=iterations - 1)

    def test_completion_tracker_overhead(self):
        """测试完成追踪器开销"""
        # 测试事件统计（event_stats）追踪机制的额外开销
        engine = TimeControlledEventEngine(
            name="CompletionTrackerEngine",
            mode=EXECUTION_MODE.BACKTEST
        )

        # 验证event_stats初始状态
        stats = engine.event_stats
        assert stats['total_events'] == 0, "初始total_events应为0"
        assert stats['completed_events'] == 0, "初始completed_events应为0"
        assert stats['failed_events'] == 0, "初始failed_events应为0"
        assert stats['processing_start_time'] is None, "初始processing_start_time应为None"

        engine.start()
        try:
            # 发送一批事件
            event_count = 50
            base_time = dt(2023, 1, 1, 10, 0, 0, tzinfo=timezone.utc)
            for i in range(event_count):
                tick = Tick(
                    code="000001.SZ",
                    price=10.0 + i * 0.01,
                ...
                price_event = EventPriceUpdate(payload=tick, timestamp=base_time + timedelta(minutes=i))
                engine.put(price_event)

            time_module.sleep(0.2)

            # 验证统计信息被正确追踪
            stats_after = engine.event_stats
            assert stats_after['total_events'] >= event_count, f"total_events应>= {event_count}"
            assert stats_after['processing_start_time'] is not None, "processing_start_time应被设置"
        finally:
            try:
                engine.stop()
            except Exception:
    ...
    def test_event_enhancement_overhead(self):
        """测试事件增强开销"""
        # 测试事件增强功能（_enhanced_processing_enabled, _event_sequence_number）的开销
        engine = TimeControlledEventEngine(
            name="EventEnhanceEngine",
            mode=EXECUTION_MODE.BACKTEST
        )

        # 验证增强处理默认开启
        assert engine._enhanced_processing_enabled is True, "增强处理默认应开启"
        assert engine._event_sequence_number == 0, "初始序列号应为0"
        ...
        try:
            ...
            elapsed = t_end - t_start
            avg_overhead = elapsed / event_count

            # 验证每次事件增强处理的平均开销在合理范围内（<50ms）
            assert avg_overhead < 0.05, f"平均事件增强开销{avg_overhead*1000:.3f}ms，应小于50ms"

            # 验证序列号在处理中被递增
            final_seq = engine._event_sequence_number
            assert final_seq >= 0, "序列号应为非负数"
        finally:
            try:
                engine.stop()
            except Exception:
