...
class DummyPortfolio:
    ...
    def bind_engine(self, engine):
        """BaseEngine.add_portfolio 要求 portfolio 实现 bind_engine"""
...
class TestBaseEngineConstruction:
    ...
    def test_backtest_base_inheritance(self):
        """BaseEngine 应继承 NamedMixin 和 ABC，并设置组件类型"""
        engine = DummyEngine()

        from ginkgo.entities.mixins import NamedMixin
        assert isinstance(engine, NamedMixin)
...
class TestBaseEngineProperties:
    ...
    def test_status_property(self):
        """Active 状态应在 start/stop 中切换（status 返回小写字符串）"""
    ...
    def test_config_property(self):
        ...
        assert hasattr(engine, 'run_sequence')

    def test_mode_property(self):
        """mode setter 应接收 EXECUTION_MODE 枚举"""
        engine = DummyEngine()
        engine.mode = EXECUTION_MODE.PAPER
        assert engine.mode is EXECUTION_MODE.PAPER
...
class TestBaseEngineIdentityManagement:
    ...
    def test_engine_id_generation_default(self, monkeypatch):
        ...
        monkeypatch.setattr(
            "ginkgo.entities.identity.IdentityUtils.generate_component_uuid",
            fake_generate,
        )

        engine = DummyEngine()

        # engine_id uses generate_component_uuid("engine")
        assert "engine_fake_0" in engine.engine_id
        # uuid is a plain uuid4(), not generated by IdentityUtils
        assert len(engine.uuid) == 36  # standard UUID format
...
class TestBaseEngineComponentManagement:
    ...
    def test_event_queue_initialization(self):
        """事件队列功能已移动到 EventEngine，BaseEngine 不直接管理队列"""
        engine = DummyEngine()
        # BaseEngine 不再持有 _event_queue
        assert not hasattr(engine, '_event_queue')
        # 但 EventEngine 子类会提供队列功能
        assert hasattr(engine, 'portfolios')
...
class TestEngineMode:
    ...
    def test_engine_mode_enum_values(self):
        ...
        assert EXECUTION_MODE.PAPER.value == 2
    ...
    def test_simulation_mode_setting(self):
        """可切换到 PAPER 模式（原 SIMULATION 已重命名为 PAPER）"""
        engine = DummyEngine(mode=EXECUTION_MODE.PAPER)
        assert engine.mode is EXECUTION_MODE.PAPER
