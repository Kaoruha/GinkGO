...
class TestEventEngineConstruction:
    ...
    def test_custom_mode_constructor(self):
        ...
        # 测试PAPER模式
        paper_engine = EventEngine(mode=EXECUTION_MODE.PAPER)
        assert paper_engine.mode == EXECUTION_MODE.PAPER
        assert paper_engine.name == "EventEngine"
        assert paper_engine._timer_interval == 1
...
class TestEventEngineProperties:
    ...
    def test_datafeeder_property(self):
        ...
        # 创建BaseFeeder实例并补充bind_engine方法（bind_datafeeder要求）
        feeder = BaseFeeder(name="test_feeder")
        feeder.bind_engine = lambda eng: None  # bind_datafeeder要求此方法存在
...
class TestEventEngineComponentBinding:
    ...
    def test_bind_matchmaking(self):
        ...
        # 补充bind_engine方法（EventEngine.bind_matchmaking要求但MatchMakingBase未实现）
        matchmaking.bind_engine = lambda eng: None
    ...
    def test_bind_portfolio(self):
        """测试绑定投资组合"""
        engine = EventEngine()
        # EventEngine没有_time_provider属性，但PortfolioBase.bind_engine会访问它
        # 这是源代码兼容性问题，通过初始化解决
        engine._time_provider = None
    ...
    def test_component_binding_coordination(self):
        ...
        class MockComponent:
            ...
            def bind_data_feeder(self, feeder):
                binding_log.append(f"{self.name}_datafeeder_bound")
        ...
        # 3. 测试Portfolio绑定协调
        # EventEngine没有_time_provider属性，PortfolioBase.bind_engine会访问它
        engine._time_provider = None
        engine.add_portfolio(portfolio)
        # add_portfolio内部已调用bind_engine + set_event_publisher + bind_data_feeder
