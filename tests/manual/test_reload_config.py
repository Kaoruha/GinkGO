...
def test_reload_api_usage():
    """演示 reload_config API 的使用方式"""
    print("\n=== reload_config() 使用方式 ===\n")
...
# 修改配置文件 ~/.ginkgo/task_timer.yml 后
timer.reload_config()  # 热重载，无需重启


# 方式 2: 通过信号触发（如果集成到 LiveCore）
# 发送 SIGHUP 信号到进程，触发 reload_config()
import signal
...
# 方式 3: 定时自动重载（可选）
# 在 LiveCore 中添加一个定时重载任务
import threading
def auto_reload_loop(timer, interval=60):
    while True:
        time.sleep(interval)
        timer.reload_config()

auto_reload_thread = threading.Thread(
    target=auto_reload_loop,
    args=(timer,),
    daemon=True
)
