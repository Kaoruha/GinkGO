...
class TestIPortfolioProtocolBasic:
    ...
    def test_portfolio_required_methods(self):
        ...
        for method in required_methods:
            assert callable(getattr(portfolio, method, None)), f"投资组合必须有{method}方法"
    ...
    def test_portfolio_required_properties(self):
        ...
        # 验证必需属性
        assert getattr(portfolio, 'name', None) is not None, "投资组合必须有name属性"
        assert getattr(portfolio, 'uuid', None) is not None, "投资组合必须有uuid属性"
...
class TestIPortfolioProtocolValidation:
    ...
    @pytest.mark.parametrize("cash,frozen,should_error", [
        ...
        (Decimal('100000.0'), Decimal('100000.1'), True),
...
from tests.unit.interfaces.conftest import sample_portfolio_data
