...
def mock_engine_base():
    ...
    class MockBaseEngine:
        def __init__(self, **kwargs):
            ...
            self.engine_id = "test_engine_123"
...
class TestEngineMixinInitialization:
    ...
    def test_mixin_initialization(self, mock_engine_base):
        ...
        class TestEngine(mock_engine_base, EngineMixin):
            def __init__(self, **kwargs):
                mock_engine_base.__init__(self, **kwargs)
        ...
        assert getattr(engine, '_event_contexts', None) is not None
        assert getattr(engine, '_performance_metrics', None) is not None
        assert getattr(engine, '_event_processing_times', None) is not None
        assert getattr(engine, '_error_log', None) is not None
    ...
    def test_mixin_with_parameters(self, mock_engine_base):
        ...
        class TestEngine(mock_engine_base, EngineMixin):
            def __init__(self, **kwargs):
                mock_engine_base.__init__(self, **kwargs)
                EngineMixin.__init__(self, **kwargs)

        engine = TestEngine()

        # 验证初始化状态
        assert len(engine._event_contexts) == 0
        assert len(engine._performance_metrics.queue_size_history) == 0
...
class TestEngineMixinEventTracking:
    ...
    def test_event_context_tracking(self, mock_engine_base):
        ...
        with engine._context_lock:
            ...
            # source code uses event.__class__.__name__ for Mock objects
...
class TestEngineMixinEventHandling:
    ...
    def test_event_handling_with_errors(self, mock_engine_base):
        ...
        # 测试错误处理 - handle_event_with_monitoring re-raises exceptions
        with pytest.raises(ValueError, match="处理错误"):
            engine.handle_event_with_monitoring(mock_event)

        # handle_event_with_monitoring only tracks errors when event_id exists,
        # and mock events don't have _event_id, so errors may not be recorded
...
class TestEngineMixinStatistics:
    ...
    def test_event_statistics_collection(self, mock_engine_base):
        ...
        # 获取统计信息 - source uses event.__class__.__name__ for Mock objects
        stats = engine.get_event_statistics()
        # Mock objects have __class__.__name__ == "Mock"
        assert stats.get('Mock', 0) == 8
    ...
    def test_statistics_with_different_distributions(self, mock_engine_base, event_counts, expected_total):
        ...
        # 按照给定分布创建事件 - use real classes so __class__.__name__ matches
        for event_type, count in event_counts.items():
            for _ in range(count):
                event = type(event_type, (), {'code': '000001.SZ'})()
                engine._track_event_start(event)
