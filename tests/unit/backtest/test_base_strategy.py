...
@pytest.fixture
def base_strategy():
    """Create a BaseStrategy instance for testing."""
    ...
    strategy.set_time_provider(mock_tp)
    strategy.advance_time(test_time)
    return strategy
...
def _make_signal(code="TEST001", direction=DIRECTION_TYPES.LONG, reason="test_signal"):
...
class TestStrategyConstruction:
    ...
    def test_inheritance_from_base(self, base_strategy):
        ...
        assert hasattr(base_strategy, "get_current_time")
...
class TestTimeManagement:
    ...
    def test_time_management(self, base_strategy):
        ...
        base_strategy.advance_time(test_time)
        assert base_strategy.current_timestamp == test_time
...
class TestStrategyPolymorphism:
    ...
    def test_strategy_with_different_directions(self, direction, reason):
        ...
        class DirectionStrategy(BaseStrategy):
            def cal(self, portfolio_info, event, *args, **kwargs):
                return [_make_signal(direction=direction, reason=reason)]
...
class TestEngineIdAccess:
    ...
    def test_engine_id_access(self, base_strategy):
        ...
        assert base_strategy.engine_id is None
...
class TestRepresentation:
    ...
    def test_strategy_repr_method(self, base_strategy):
        ...
        assert "BaseStrategy" in repr_str
...
class TestMultipleSignals:
    ...
    def test_multiple_signals_generation(self, portfolio_info, mock_event):
        ...
        class MultiSignalStrategy(BaseStrategy):
            def cal(self, portfolio_info, event, *args, **kwargs):
                return [
                    _make_signal(code="TEST001", reason="Buy signal"),
                    _make_signal(code="TEST002", direction=DIRECTION_TYPES.SHORT, reason="Sell signal"),
                ]
