...
class TestVolatility(unittest.TestCase):
    ...
    def setUp(self):
        """初始化测试用的Volatility实例"""
        self.analyzer = Volatility("test_volatility", window=20)
    ...
    def test_init(self):
        ...
        self.assertEqual(analyzer._window, 20)  # 默认窗口大小
    ...
    def test_custom_window(self):
        """测试自定义窗口大小"""
        analyzer = Volatility(window=30)
        self.assertEqual(analyzer._window, 30)
    ...
    def test_rolling_window_calculation(self):
        ...
        # 第1天（index 0）初始化 _last_worth，波动率为0
        self.assertEqual(volatilities[0], 0.0)

        # 第2天（index 1）有1个收益率数据点，不够2个，波动率为0
        self.assertEqual(volatilities[1], 0.0)

        # 第3天起（index 2+）有2+个数据点，波动率>0
        for i in range(2, len(volatilities)):
            self.assertGreater(volatilities[i], 0)

        # 随着窗口滑动，波动率应该有变化
        window_volatilities = volatilities[2:]
        self.assertGreater(len(set(window_volatilities)), 1)  # 不应该都相同

    def test_window_effect(self):
        """测试不同窗口大小的影响"""
        analyzers = {
            10: Volatility(window=10),
            30: Volatility(window=30)
        }
        ...
        for window_size, analyzer in analyzers.items():
            # 需要设置 time_provider 才能让 advance_time 和 add_data 工作
    ...
    def test_properties(self):
        ...
        # 测试日波动率
        daily_vol = self.analyzer.daily_volatility
        self.assertIsInstance(daily_vol, float)
        self.assertGreaterEqual(daily_vol, 0)

        # 年化波动率应该约等于 日波动率 * sqrt(252)
        if daily_vol > 0:
            expected_annual = daily_vol * np.sqrt(252)
            self.assertAlmostEqual(volatility, expected_annual, places=1)
