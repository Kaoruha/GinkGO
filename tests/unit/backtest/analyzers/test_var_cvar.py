...
class TestVarCVar(unittest.TestCase):
    ...
    def test_properties(self):
        ...
        # 测试置信度属性
        conf = self.analyzer.confidence_level
        self.assertEqual(conf, 0.95)

        # 测试尾部风险比例
        tail_ratio = self.analyzer.tail_risk_ratio
        self.assertIsInstance(tail_ratio, float)

        # 测试收益率数量
        ret_count = self.analyzer.returns_count
        self.assertIsInstance(ret_count, int)
        self.assertGreater(ret_count, 0)
