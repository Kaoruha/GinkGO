...
class TestExponentialMovingAverageBoundaryValues:
    ...
    @pytest.mark.parametrize("period", [2, 3, 5, 10, 20, 50])
    ...
    def test_ema_period_one(self):
        """Test EMA with period=1 returns the only price itself."""
        # EMA.cal requires len(prices) == period; period=1 needs exactly 1 price
        prices = [100.0]
        result = ExponentialMovingAverage.cal(1, prices)
        assert not pd.isna(result)
        # With period=1 and a single price, EMA equals that price
        assert result == 100.0
