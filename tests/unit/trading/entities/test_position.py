...
class TestPositionProperties:
    ...
    def test_newly_implemented_properties(self):
        ...
        # last_update需要TimeProvider才能自动更新，无TimeProvider时为None
        # 但last_update属性存在且可手动设置
        custom_time = datetime.datetime(2024, 6, 1, 10, 0, 0)
        position.last_update = custom_time
...
class TestPositionAttributeUpdates:
    ...
    def test_constructor_with_new_attributes(self):
        ...
        # 验证新属性被正确设置
        # init_time由__init__覆盖为当前时间（不受构造参数影响）
        assert isinstance(position.init_time, datetime.datetime)
        assert position.init_time is not None
        # last_update无TimeProvider时为None，但属性存在且可设置
        assert hasattr(position, 'last_update')
        position.last_update = custom_last_update
        assert isinstance(position.last_update, datetime.datetime)
        assert position.last_update == custom_last_update
    ...
    def test_set_method_with_new_attributes(self):
        ...
        # last_update无TimeProvider时自动更新返回None（_update_last_update内部调用get_current_time()返回None）
        # 即使通过set()传入last_update值，也被_update_last_update覆盖为None
    ...
    def test_automatic_last_update_on_operations(self):
        ...
        # last_update需要TimeProvider才能自动更新
        # 无TimeProvider时last_update保持None，但操作本身成功
        # 手动设置last_update来验证操作不影响它
        position.last_update = datetime.datetime(2024, 1, 1, 10, 0, 0)

        # 模拟交易操作
        position.deal(DIRECTION_TYPES.LONG, 120.0, 500)

        # 验证交易成功（last_update在没有TimeProvider时不会自动更新）
    ...
    def test_add_realized_pnl_method(self):
        ...
        # last_update需要TimeProvider才能自动更新，无TimeProvider时保持None
    ...
    def test_set_without_last_update_auto_updates(self):
        ...
        # last_update无TimeProvider时为None
...
class TestPositionTradingRegimes:
    ...
    def test_settlement_days_configuration(self):
        ...
        assert position_t0.settlement_days == 0
        assert position_t0.settlement_frozen_volume == 0

        # 测试T+1模式（通过构造函数设置，settlement_days无setter）
        ...
        assert position_t1.settlement_days == 1
        ...
        assert position_t2.settlement_days == 2
    ...
    @pytest.mark.xfail(reason="settlement_days无setter且advance_time()需要TimeProvider（源代码限制）")
    ...
    @pytest.mark.xfail(reason="settlement_days无setter（源代码限制）")
