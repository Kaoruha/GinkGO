"""
LiveFeederBase 端到端：装 EngineBindableMixin+FeederPublishMixin 后 MRO 通 + stats 合并。

验证关键风险点（ADR-019 Consequences）：
- stats 合并：LiveFeederBase 自有 stats 字段与 FeederPublishMixin.stats 重叠，
...
桩用 __abstractmethods__ = frozenset() 绕过 LiveDataFeeder ABC，不依赖 GCONF/WS。
"""
...
class _ConcreteLiveFeederBase(LiveFeederBase):
    """桩：绕过 LiveDataFeeder 抽象方法，仅为测 LiveFeederBase 的 mixin MRO + stats 合并。"""


_ConcreteLiveFeederBase.__abstractmethods__ = frozenset()


def _make_feeder():
    return _ConcreteLiveFeederBase()


class TestLiveFeederBaseStatsMerge:
    def test_stats_keeps_both_publish_mixin_and_feeder_fields(self):
        """stats 合并：保留 FeederPublishMixin 的 events_published/publish_errors + LiveFeederBase 特有字段。

        关键：LiveFeederBase.__init__ 的 self.stats 不能覆盖 FeederPublishMixin 设的 stats
        （否则丢 publish_errors → runtime 抛时 stats["publish_errors"] += 1 触发 KeyError）。
        """
        feeder = _make_feeder()
        assert "events_published" in feeder.stats
        assert "publish_errors" in feeder.stats, "FeederPublishMixin 字段未被 LiveFeederBase.stats 覆盖"
        assert "messages_received" in feeder.stats
        assert "connection_errors" in feeder.stats


class TestLiveFeederBaseEventPublisherBinding:
...
class TestLiveFeederBasePublishPriceUpdate:
