...
class TestTimeControlledEngineProperties:
    ...
    @pytest.mark.skip(reason="config property not implemented on TimeControlledEventEngine")
    ...
    def test_time_provider_property(self):
        ...
        assert isinstance(paper_engine._time_provider, SystemTimeProvider), "PAPER模式应使用SystemTimeProvider"
...
class TestEngineConfigManagement:
    ...
    def test_engine_config_creation(self):
        ...
        # 验证基本配置通过公共接口可访问
        engine_status = engine.get_engine_status()
        assert engine.name == "TestEngine", "引擎名称应正确设置"
        assert engine.mode == EXECUTION_MODE.BACKTEST, "执行模式应正确设置"
    ...
    def test_event_queue_size_configuration(self):
        ...
        # 等待异步队列调整完成（set_event_queue_size使用后台线程）
    ...
    def test_timeout_configuration(self):
        ...
        # 测试默认超时配置
        default_engine = TimeControlledEventEngine()
        assert default_engine.event_timeout == 30.0, "默认超时应为30秒"
    ...
    @pytest.mark.skip(reason="priority parameter not implemented on TimeControlledEventEngine")
...
class TestTimeControllerValidation:
    ...
    def test_time_mode_consistency(self):
        ...
        # PAPER模式使用系统时间（与LIVE一致）
        paper_engine = TimeControlledEventEngine(mode=EXECUTION_MODE.PAPER)
        paper_time_info = paper_engine.get_time_info()
        assert paper_time_info.time_mode == TIME_MODE.SYSTEM, "PAPER模式应使用SYSTEM时间"
        assert paper_time_info.is_logical_time == False, "PAPER模式应标记为实时时间"
    ...
    def test_config_parameter_validation(self):
        ...
        valid_configs = [
            ...
            {"timer_interval": 1.0, "max_event_queue_size": 1000, "event_timeout_seconds": 5.0}
        ]
    ...
    def test_timeout_value_validation(self):
        ...
        min_timeout = 0.001
        max_timeout = 300.0
        engine_min = TimeControlledEventEngine(event_timeout_seconds=min_timeout)
        engine_max = TimeControlledEventEngine(event_timeout_seconds=max_timeout)
