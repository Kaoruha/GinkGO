...
class TestEventHandling:
    ...
    def test_event_handler_execution_order(self):
        ...
        # 验证自定义处理器都被调用（引擎内置处理器也会被调用）
        assert len(execution_order) >= 3, "应至少有3个自定义处理器被调用"
    ...
    def test_event_payload_access(self):
        ...
        price_event = EventPriceUpdate(payload=tick)
...
class TestTimeAdvancement:
    ...
    def test_time_advancement_integration(self):
        ...
        engine._time_provider.set_current_time(start_time)
        ...
        # 验证时间推进记录（advance_time_to 也会触发内部事件，所以只过滤我们投递的事件）
        our_targets = {tp for tp in time_points}
        our_advancements = [log for log in advancement_log if log['target_time'] in our_targets]
        assert len(our_advancements) == len(time_points), "应记录所有我们投递的时间推进"
...
class TestEventQueueOperations:
    ...
    def test_event_queue_initialization(self):
        ...
        assert engine._event_queue.empty(), "队列初始应为空"
    ...
    def test_event_queue_thread_safety(self):
        ...
        def dequeue_worker(thread_id):
            ...
            for i in range(10):
                try:
                    # 安全获取事件
                    event = engine._event_queue.get(timeout=0.1)
                    if event:
                        with operation_lock:
                            dequeue_count += 1
                except:
