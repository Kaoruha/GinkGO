...
from ginkgo.trading.feeders.live_feeder import LiveFeederBase, RateLimiter, ConnectionManager
...
class TestLiveFeederInitialization:
    ...
    def test_initialize_sets_rate_limiter(self):
        feeder = LiveFeederBase(enable_rate_limit=True, rate_limit_per_second=20.0)
...
class TestConnectionLifecycle:
    ...
    def test_start_launches_event_loop_thread(self):
        feeder = LiveFeederBase(host="localhost", port=8080)
        feeder.initialize()
...
class TestSubscriptionManagement:
    ...
    def test_subscribe_symbols_rate_limit(self):
        feeder = LiveFeederBase(enable_rate_limit=True, rate_limit_per_second=0.0001)
        feeder.initialize()
