...
class TestDefaultAnalyzerSet:
    ...
    def test_minimal_set_exists(self):
        """测试MINIMAL集合存在"""
        assert getattr(DEFAULT_ANALYZER_SET, 'MINIMAL', None) is not None
        assert DEFAULT_ANALYZER_SET.MINIMAL.value == 1

    def test_standard_set_exists(self):
        """测试STANDARD集合存在"""
        assert getattr(DEFAULT_ANALYZER_SET, 'STANDARD', None) is not None
        assert DEFAULT_ANALYZER_SET.STANDARD.value == 2

    def test_full_set_exists(self):
        """测试FULL集合存在"""
        assert getattr(DEFAULT_ANALYZER_SET, 'FULL', None) is not None
