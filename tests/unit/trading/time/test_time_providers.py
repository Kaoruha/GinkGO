...
class TestLogicalTimeProviderListenerManagement:
    ...
    def test_register_time_listener(self):
        ...
        assert listener1 in provider._listeners
        assert listener2 in provider._listeners
    ...
    def test_unregister_time_listener(self):
        ...
        # 注销监听器
        provider.unregister_time_listener(listener)
        assert listener not in provider._listeners
    ...
    def test_listener_notification_on_time_change(self):
        """测试时间变化时通知监听器"""
        # 注意：当前LogicalTimeProvider实现不主动通知监听器，
        # set_current_time只更新时间。监听器集合仅用于注册/注销管理。
        # 验证set_current_time更新时间后监听器集合不变
        initial_time = datetime(2023, 1, 1, 10, 0, 0, tzinfo=timezone.utc)
        provider = LogicalTimeProvider(initial_time)
    ...
    def test_listener_notification_exception_handling(self):
        """测试监听器异常处理"""
        # 当前实现不主动调用监听器回调，set_current_time是纯状态更新。
        # 验证即使注册了多个监听器，set_current_time仍能正常工作
        initial_time = datetime(2023, 1, 1, 10, 0, 0, tzinfo=timezone.utc)
        provider = LogicalTimeProvider(initial_time)
        ...
        # 正常推进时间
        new_time = datetime(2023, 1, 5, 10, 0, 0, tzinfo=timezone.utc)
        result = provider.set_current_time(new_time)
    ...
    def test_multiple_listeners_notification(self):
        ...
        # 验证所有监听器都在集合中
        assert len(provider._listeners) == 5
        for listener in listeners:
            assert listener in provider._listeners

        # Set中重复注册不增加数量
        provider.register_time_listener(listeners[0])
        assert len(provider._listeners) == 5
...
class TestLogicalTimeProviderHeartbeat:
    ...
    def test_start_heartbeat(self):
        """测试启动心跳"""
        # 心跳功能在SystemTimeProvider上实现，LogicalTimeProvider不支持
        # 验证LogicalTimeProvider没有heartbeat相关属性/方法
        initial_time = datetime(2023, 1, 1, 10, 0, 0, tzinfo=timezone.utc)
        provider = LogicalTimeProvider(initial_time)
        assert not hasattr(provider, '_heartbeat_active') or not getattr(provider, '_heartbeat_active', False)

    def test_stop_heartbeat(self):
        """测试停止心跳"""
        # LogicalTimeProvider没有heartbeat功能
        initial_time = datetime(2023, 1, 1, 10, 0, 0, tzinfo=timezone.utc)
        provider = LogicalTimeProvider(initial_time)
        # 验证基础时间推进功能正常
        new_time = datetime(2023, 1, 2, 10, 0, 0, tzinfo=timezone.utc)
        provider.advance_time_to(new_time)
        assert provider.now() == new_time

    def test_heartbeat_interval_parameter(self):
        """测试心跳间隔参数"""
        # LogicalTimeProvider不需要心跳，时间由外部控制推进
        # 验证时间推进完全由外部控制
        initial_time = datetime(2023, 1, 1, 10, 0, 0, tzinfo=timezone.utc)
        provider = LogicalTimeProvider(initial_time)

        # 连续推进多次，验证每次都精确更新
        t1 = datetime(2023, 1, 1, 11, 0, 0, tzinfo=timezone.utc)
        t2 = datetime(2023, 1, 1, 12, 0, 0, tzinfo=timezone.utc)
        t3 = datetime(2023, 1, 2, 9, 30, 0, tzinfo=timezone.utc)
        provider.advance_time_to(t1)
        assert provider.now() == t1
        provider.advance_time_to(t2)
        assert provider.now() == t2
        provider.advance_time_to(t3)
        assert provider.now() == t3
...
class TestSystemTimeProviderHeartbeat:
    ...
    def test_start_heartbeat(self):
        ...
        # 验证初始状态心跳未激活
        assert provider._heartbeat_active is False

        # 启动心跳
        provider.start_heartbeat()
        assert provider._heartbeat_active is True
    ...
    def test_heartbeat_interval_parameter(self):
        ...
        # 停止后使用自定义间隔重启
        provider.stop_heartbeat()
        provider.start_heartbeat(interval_seconds=0.5)
        assert provider._heartbeat_active is True
...
class TestDSTHandlerTimezoneNormalization:
    ...
    def test_normalize_timezone_with_timezone(self):
        ...
        # 转换到东京时区
        result = DSTHandler.normalize_timezone(utc_time, tokyo_tz)

        # 验证转换结果有时区信息
        assert result.tzinfo is not None
        # 验证时间被正确转换（UTC 12:00 -> Tokyo 21:00）
    ...
    def test_normalize_timezone_without_timezone(self):
        ...
        import pytz
        # 创建无时区的时间
        naive_time = datetime(2023, 6, 15, 12, 0, 0)
        assert naive_time.tzinfo is None

        shanghai_tz = pytz.timezone('Asia/Shanghai')

        # 转换到上海时区（先添加UTC，再转换）
        result = DSTHandler.normalize_timezone(naive_time, shanghai_tz)

        # 验证结果有时区信息
        assert result.tzinfo is not None
        # 验证时间被正确转换（UTC 12:00 -> Shanghai 20:00）
    ...
    def test_normalize_timezone_utc_to_eastern(self):
        ...
        # 夏季：UTC 14:00 -> Eastern 10:00 (EDT, UTC-4)
        summer_utc = datetime(2023, 7, 1, 14, 0, 0, tzinfo=timezone.utc)
        eastern_summer = DSTHandler.normalize_timezone(summer_utc, eastern_tz)
        assert eastern_summer.hour == 10

        # 冬季：UTC 14:00 -> Eastern 9:00 (EST, UTC-5)
        winter_utc = datetime(2023, 1, 1, 14, 0, 0, tzinfo=timezone.utc)
        eastern_winter = DSTHandler.normalize_timezone(winter_utc, eastern_tz)
...
class TestDSTHandlerDSTTransition:
    ...
    def test_handle_dst_transition_with_pytz(self):
        ...
        # 夏季时间（无歧义）
        summer_time = datetime(2023, 7, 1, 10, 0, 0)
        result = DSTHandler.handle_dst_transition(summer_time, 'US/Eastern')
    ...
    def test_handle_dst_transition_spring_forward(self):
        """测试春季夏令时前进（跳过1小时）"""
        import pytz
        # 美国夏令时在3月第二个星期日凌晨2点跳到3点
        # 2023年3月12日 2:30AM不存在（春季前进）
        spring_time = datetime(2023, 3, 12, 2, 30, 0)

        # handle_dst_transition内部捕获pytz异常并重新抛出ValueError
        with pytest.raises(ValueError, match="DST handling failed"):
            DSTHandler.handle_dst_transition(spring_time, 'US/Eastern')

    def test_handle_dst_transition_fall_back(self):
        """测试秋季夏令时后退（重复1小时）"""
        import pytz
        # 美国夏令时在11月第一个星期日凌晨2点回退到1点
        # 2023年11月5日 1:30AM是模糊时间（出现两次）
        fall_time = datetime(2023, 11, 5, 1, 30, 0)

        # handle_dst_transition内部捕获pytz异常并重新抛出ValueError
        with pytest.raises(ValueError, match="DST handling failed"):
            DSTHandler.handle_dst_transition(fall_time, 'US/Eastern')

    def test_handle_dst_invalid_timezone_raises_error(self):
        """测试无效时区抛出错误"""
        invalid_time = datetime(2023, 1, 1, 10, 0, 0)

        # 验证无效时区抛出ValueError
        with pytest.raises(ValueError, match="DST handling failed"):
            DSTHandler.handle_dst_transition(invalid_time, 'Invalid/Timezone')
...
class TestTimeProvidersIntegration:
    ...
    def test_logical_provider_with_boundary_validator(self):
        ...
        # 模拟回测场景：推进时间 -> 验证数据访问 -> 防未来数据泄露
        initial_time = datetime(2023, 1, 1, 10, 0, 0, tzinfo=timezone.utc)
        provider = LogicalTimeProvider(initial_time)
        validator = TimeBoundaryValidator(provider)

        # 当前时间10:00，可以访问当天10:00之前的数据
        same_time_data = datetime(2023, 1, 1, 10, 0, 0, tzinfo=timezone.utc)
        validator.validate_data_access(same_time_data)  # 不抛异常（当前时间点）

        # 不能访问当天10:00之后的数据（未来数据）
        future_same_day = datetime(2023, 1, 1, 15, 0, 0, tzinfo=timezone.utc)
        with pytest.raises(ValueError):
            validator.validate_data_access(future_same_day)

        # 推进到当天收盘
        day1_close = datetime(2023, 1, 1, 15, 0, 0, tzinfo=timezone.utc)
        provider.advance_time_to(day1_close)
        # 现在可以访问当天收盘数据了
        validator.validate_data_access(day1_close)  # 不抛异常
        validator.validate_data_access(same_time_data)  # 历史数据也可以

        # 推进到第二天
        day2 = datetime(2023, 1, 2, 10, 0, 0, tzinfo=timezone.utc)
        provider.advance_time_to(day2)

        # 可以访问第一天数据（历史数据）
        validator.validate_data_access(day1_close)  # 不抛异常
        # 可以访问第二天当前时间之前的数据
        validator.validate_data_access(day2)  # 不抛异常
        # 不能访问第二天之后的数据
        day3_data = datetime(2023, 1, 3, 10, 0, 0, tzinfo=timezone.utc)
        with pytest.raises(ValueError):
            validator.validate_data_access(day3_data)

        # 轻量级检查
        assert validator.can_access_time(day1_close) is True
        assert validator.can_access_time(day2) is True
        assert validator.can_access_time(day3_data) is False
    ...
    def test_time_provider_polymorphism(self):
        ...
        for provider in [logical, system]:
            assert callable(getattr(provider, 'now', None))
            assert callable(getattr(provider, 'get_mode', None))
            assert callable(getattr(provider, 'set_current_time', None))
            assert callable(getattr(provider, 'advance_time_to', None))
            assert callable(getattr(provider, 'can_access_time', None))

        # 验证多态行为：LogicalTimeProvider返回逻辑时间
        assert logical.get_mode() == TIME_MODE.LOGICAL
        assert logical.now() == datetime(2023, 1, 1, 10, 0, 0, tzinfo=timezone.utc)

        # 验证多态行为：SystemTimeProvider返回系统时间
        assert system.get_mode() == TIME_MODE.SYSTEM
        sys_time = system.now()
        assert sys_time.tzinfo == timezone.utc
    ...
    def test_timezone_consistency_across_components(self):
        ...
        # 使用上海时区创建LogicalTimeProvider
        initial = datetime(2023, 6, 1, 10, 0, 0)
        provider = LogicalTimeProvider(initial, timezone_info=shanghai_tz)
        validator = TimeBoundaryValidator(provider)

        # 验证provider和validator使用相同时区
        assert provider._timezone == shanghai_tz
        assert provider.now().tzinfo == shanghai_tz

        # 验证数据访问验证与时区一致
        past = datetime(2023, 5, 1, 10, 0, 0)  # naive时间，会被自动添加时区
        assert validator.can_access_time(past) is True

        future = datetime(2023, 7, 1, 10, 0, 0)  # naive时间，会被自动添加时区
        assert validator.can_access_time(future) is False

        # 验证DSTHandler与provider时区一致
        utc_time = datetime(2023, 6, 1, 2, 0, 0, tzinfo=timezone.utc)
        converted = DSTHandler.normalize_timezone(utc_time, shanghai_tz)
