...
class TestConstruction:
    def test_empty_data_raises(self):
        with pytest.raises(ValueError, match="无任何可用数据"):
            RollingReport(task_id="test", data=DataProvider())

    def test_data_shorter_than_window(self):
        dp = _make_dp(("net_value", _make_df(list(range(10)))))
        report = RollingReport(task_id="test", data=dp, window=60)
    ...
    def test_accepts_analyzers_filter(self):
        ...
        report = RollingReport(task_id="test", data=dp, window=60, analyzers=["net_value"])
        d = report.to_dict()
    ...
    def test_analyzers_none_processes_all(self):
        ...
        report = RollingReport(task_id="test", data=dp, window=60, analyzers=None)
        d = report.to_dict()
        for date_metrics in d.values():
            assert "net_value" in date_metrics
    ...
    def test_missing_analyzer_skipped(self):
        ...
        report = RollingReport(task_id="test", data=dp, window=60, analyzers=["nonexistent"])
        assert report.to_dict() == {}

    def test_no_timestamp_skipped(self):
        dp = DataProvider()
        dp.add("bad", pd.DataFrame({"value": [1.0, 2.0, 3.0]}))
        report = RollingReport(task_id="test", data=dp, window=2)
        assert report.to_dict() == {}
...
class TestWindowComputation:
    def test_sliding_window_count(self):
        ...
        dp = _make_dp(("net_value", _make_df(list(range(120)))))
        report = RollingReport(task_id="test", data=dp, window=60, step=1)
        d = report.to_dict()
        assert len(d) == 61
    ...
    def test_tumbling_window_count(self):
        ...
        dp = _make_dp(("net_value", _make_df(list(range(120)))))
        report = RollingReport(task_id="test", data=dp, window=60, step=60)
        d = report.to_dict()
        assert len(d) == 2

    def test_window_stats_structure(self):
        dp = _make_dp(("net_value", _make_df(list(range(100)))))
        report = RollingReport(task_id="test", data=dp, window=10)
        d = report.to_dict()
    ...
    def test_window_values_correct(self):
        ...
        dp = _make_dp(("net_value", _make_df(list(range(100)))))
        report = RollingReport(task_id="test", data=dp, window=10, step=1)
        d = report.to_dict()
        first_key = list(d.keys())[0]
        stats = d[first_key]["net_value"]
