...
class TestEngineHandlerMappingCRUDConversion:
    ...
    def test_data_count_changes_with_conversion(self):
        ...
        unique_prefix = f"count_test_{_uuid.uuid4().hex[:8]}_"

        try:
            # 清理可能存在的旧测试数据
            mapping_crud.remove(filters={"engine_id__like": f"{unique_prefix}%"})

            # 记录初始数据条数（使用足够大的 page_size 避免截断）
            initial_count = len(mapping_crud.find(page_size=100000))
            ...
            for i in range(3):
                mapping = MEngineHandlerMapping(
                    engine_id=f"{unique_prefix}engine_{i+1:03d}",
                    handler_id=f"{unique_prefix}handler_{i+1:03d}",
                    type=EVENT_TYPES.PRICEUPDATE,
            ...
            after_insert_count = len(mapping_crud.find(page_size=100000))
            ...
            after_delete_count = len(mapping_crud.find(page_size=100000))
            ...
            remaining_count = len(mapping_crud.find(filters={
                "engine_id__like": f"{unique_prefix}%"
            }))
