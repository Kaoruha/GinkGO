...
class TestBarCRUDReplace:
    ...
    def test_replace_batch_performance(self):
        ...
        new_bars = [
            MBar(code="BATCH.SZ", timestamp=datetime(2023, 12, 1 + i, 9, 30),
                 open=Decimal(str(100 + i)), close=Decimal(str(101 + i)),
                 high=Decimal(str(102 + i)), low=Decimal(str(99 + i)),
    ...
    def test_replace_cross_database(self):
        ...
        assert hasattr(bar, 'close'), "MBar应有close属性"
        assert hasattr(bar, 'volume'), "MBar应有volume属性"
