...
class TestSignalCRUDQuery:
    ...
    def test_model_list_conversions(self):
        ...
        # 使用唯一 portfolio_id 避免与其他测试冲突
        unique_portfolio_id = f"convert_test_{_uuid.uuid4().hex[:8]}"
        ...
        try:
            ...
            # 验证插入前后数据变化
            print("\n→ 验证插入前数据...")
            before_count = len(signal_crud.find(filters={"portfolio_id": unique_portfolio_id}))
            print(f"✓ 插入前数据量: {before_count}")

            # 第二次插入 - 创建新对象以获得新 UUID
            print(f"\n→ 再次插入 {len(test_signals)} 条测试数据...")
            second_batch = [
                MSignal(
                    portfolio_id=unique_portfolio_id,
            ...
            signal_crud.add_batch(second_batch)
            print("✓ 数据插入成功")

            # 验证插入后的数据变化
            print("\n→ 验证插入后数据变化...")
            after_count = len(signal_crud.find(filters={"portfolio_id": unique_portfolio_id}))
            ...
            # 清理测试数据并验证删除效果
            print("\n→ 清理测试数据...")
            before_delete = len(signal_crud.find(filters={"portfolio_id": unique_portfolio_id}))
            print(f"✓ 删除前数据量: {before_delete}")

            signal_crud.remove(filters={"portfolio_id": unique_portfolio_id})

            after_delete = len(signal_crud.find(filters={"portfolio_id": unique_portfolio_id}))
    ...
    def test_find_by_business_time_range(self):
        ...
        try:
            ...
            for signal in business_time_signals:
                ...
                if signal.business_timestamp and signal.business_timestamp.year > 1970:
    ...
    def test_signal_data_integrity(self):
        ...
        try:
            ...
            try:
                ...
                result = signal_crud.add(invalid_signal)
                assert result is not None, "空portfolio_id的信号应能成功插入"
                print(f"✓ 空portfolio_id信号插入成功（源码当前允许空值）")
            except Exception as e:
