...
向后兼容 get()         -> 不再透传裸 list（CRUD 直接返 list，ADR-029 §Decision 9）

设计：用 MagicMock 桩 _crud_repo.find，返回 list（CRUD 直接返 list 形态），
断言 data 的运行时类型而非源码字面量。
"""
...
def _make_empty_list() -> list:
    """构造空 list（CRUD 直接返 list 形态）。"""
    return []
...
def test_get_stockinfos_df_returns_dataframe_empty():
    """出口①：空结果 data 是 pd.DataFrame，非 [] / None / list。"""
    svc = _make_service(_make_empty_list())
...
def test_backward_compat_get_no_longer_leaks_orm_list():
    """get() 向后兼容，data 是 List[StockInfo]（不是裸 ORM model list）。"""
    svc = _make_service(_make_empty_list())
    result = svc.get()
    assert result.success is True
    # data 是 list（Entity 列表），元素是 StockInfo（若非空）
    assert isinstance(result.data, list)
...
def test_backward_compat_get_returns_entity_list():
    ...
    现改委托 get_stockinfos()，返 List[StockInfo]，与原 list 迭代语义兼容。
    """
    svc = _make_service(_make_empty_list())
    result = svc.get()
    assert isinstance(result.data, list)
