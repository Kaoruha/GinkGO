...
class TestParamServiceBusinessMethods:
    ...
    def test_copy_params(self, param_service):
        ...
        timestamp = datetime.now().strftime('%H%M%S')
        suffix = ''.join(random.choices(string.ascii_lowercase + string.digits, k=6))
        source_mapping = f"test_{timestamp}_{suffix}"
        suffix2 = ''.join(random.choices(string.ascii_lowercase + string.digits, k=6))
        target_mapping = f"test_{timestamp}_{suffix2}"
...
class TestParamServiceEdgeCases:
    ...
    def test_various_value_types(self, param_service, unique_mapping_id, value, description):
        ...
        assert result.success
        assert result.data["param_info"]["value"] == str(value)
