...
from ginkgo.backtest.events import FillEvent
from ginkgo.backtest.enums import DealType
from ginkgo.data.ginkgo_mongo import ginkgo_mongo as gm
from ginkgo.backtest.price import DayBar, Min5Bar, CurrentPrice
...
class SimulateMatcher(BaseMatcher):
    ...
    def try_match(self, order, broker, current_price):
        """
        尝试撮合
        """
        # 尝试成交
    ...
    def get_result(self, current_price):
        ...
        for i in self._match_list:
            if i.code not in current_price.keys():
                print("未持有订单标的的价格数据，请检查代码")
                continue
            if isinstance(current_price[i.code], DayBar):
                p = current_price[i.code].data
            ...
            if isinstance(current_price[i.code], Min5Bar):
                print("Min5数据的处理，TODO")
                pass
            if isinstance(current_price[i.code], CurrentPrice):
                print("即时数据处理，TODO")
                pass
