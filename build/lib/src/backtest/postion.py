...
class Position(object):
    ...
    def per_sell(self, volume: int):
        ...
        if volume > self.volume:
            gl.warning(f'预沽量{volume}大于持仓{self.volume}，已重新设置为持仓量，请检查代码')
            volume = self.volume
        self.freeze += volume
        self.volume -= volume
    ...
    def sell(self, volume, done):
        ...
        if done:
            if volume > self.freeze:
                gl.error(f"交易成功，成交量{volume}大于冻结量{self.freeze}，请检查代码，当前回测有误")
                self.freeze = 0
            else:
                self.freeze -= volume
        else:
            if volume > self.freeze:
                gl.error(f"交易失败，但是预计成交量{volume}大于冻结量{self.freeze}，请检查代码，当前回测有误")
                self.volume += self.freeze
                self.freeze = 0
            else:
                self.volume += volume
                self.freeze -= volume
