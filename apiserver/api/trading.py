...
@router.get("/accounts", response_model=APIResponse[list])
async def list_paper_accounts():
    """获取模拟盘账户列表（PAPER 模式的 Portfolio）"""
    try:
        portfolio_crud = _get_container().cruds.portfolio()
        from ginkgo.enums import PORTFOLIO_MODE_TYPES

        # 查询所有 PAPER 模式的 portfolio
...
@router.post("/accounts", response_model=APIResponse[dict], status_code=status.HTTP_201_CREATED)
async def create_paper_account(data: CreatePaperAccountRequest):
    ...
    try:
        ...
        if not result.is_success():
            raise BusinessError(f"Failed to create paper account: {result.error_message}")

        # 获取新创建的 portfolio
        portfolio_id = None
        if isinstance(result.data, list) and result.data:
            portfolio_id = result.data[0].uuid
        elif hasattr(result.data, 'uuid'):
            portfolio_id = result.data.uuid
        elif isinstance(result.data, dict):
            portfolio_id = result.data.get('uuid')
...
@router.get("/accounts/{account_id}", response_model=APIResponse[dict])
async def get_paper_account(account_id: str):
    """获取模拟盘账户详情（含持仓和活跃订单）"""
    try:
        portfolio_crud = _get_container().cruds.portfolio()
        db_portfolios = portfolio_crud.find(filters={"uuid": account_id})

        if not db_portfolios:
            raise NotFoundError("Paper account", account_id)
...
@router.post("/{account_id}/start", response_model=APIResponse[dict])
...
@router.post("/{account_id}/stop", response_model=APIResponse[dict])
...
@router.get("/{account_id}/positions", response_model=APIResponse[list])
...
@router.get("/{account_id}/orders", response_model=APIResponse[list])
...
@router.delete("/{account_id}/orders/{order_id}", response_model=APIResponse[dict])
async def cancel_paper_order(account_id: str, order_id: str):
    ...
    try:
        ...
        order_crud = _get_container().cruds.order_record()
        order_records = order_crud.find_by_order_id(order_id)
        ...
        # 验证订单属于该 portfolio
        if order.portfolio_id != account_id:
            raise BusinessError("Order does not belong to this account")
        ...
        # 更新订单状态为 CANCELED
        # 通过 crud 的 update 方法修改状态
        try:
            order_crud.update_by_uuid(
                uuid=order_id if hasattr(order, 'uuid') else getattr(order, 'uuid', None),
                data={"status": ORDERSTATUS_TYPES.CANCELED.value}
            )
        except AttributeError:
            # 如果没有 update_by_uuid 方法，尝试通过 remove + 重新插入模拟
            raise BusinessError(
                "Order cancellation not fully supported: "
                "CRUD lacks update method for order status"
            )
...
@router.get("/{account_id}/report/daily", response_model=APIResponse[dict])
async def get_paper_report(
    ...
    try:
        ...
        from ginkgo.enums import ORDERSTATUS_TYPES
        order_crud = _get_container().cruds.order_record()
        orders = order_crud.find_by_portfolio(
        ...
        # 查询当前持仓数量
        position_crud = _get_container().cruds.position_record()
        positions = position_crud.find_current_positions(
            portfolio_id=account_id,
            min_volume=1,
        )
...
def _query_positions(account_id: str) -> list:
    ...
    try:
        position_crud = _get_container().cruds.position_record()
        records = position_crud.find_current_positions(
            portfolio_id=account_id,
            min_volume=1,
        )
